...
class DataTable
{
	...
	{
		$this->out_text.='<table id="'.$this->name.'" class="table" cellspacing="0" cellpadding="0" width="100%" border="0"><form id="'.$this->name.'_form" name="'.$this->name.'_form" method="post" action=""><tbody>'."\n";
		if($this->select_able)
		{
	...
	private function build_pager_code()
	{
		...
		$tempstr.= '<td width="190"><h3 class=title>'.$this->edit_label.'</h3></td>';
		if($this->select_able)
		$tempstr.='<td width=""><input name="button" type="button" value="删 除" class="deleteall" onclick="deleteAll(\''.$this->delete_url.'\')"/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<select name="select" onChange="move_to(this.options[this.options.selectedIndex].value,\''.$this->move_url.'\')"><option value="" selected="selected">移动到...</option>'.$this->build_options().'</select>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="text" name="dtTime" class="txt" id="dtTime" value="'.date("Y-m-d H:i:s").'">&nbsp;<input type="button" value="定时发布" onclick="deleteAll1(\''.$this->delete_url.'&move=1\')" class="deleteall"></td>';	
		else
		$tempstr.='<td width=""><h3 class=title>&nbsp;</h3></td>';
		$tempstr.= '<td width="440" align=right><h3 class=title style="font-weight:normal;"><div class="zzp-page">';
		$this->order_text= $tempstr.$this->sb->get_pager_show().'</div></h3></td></tr></table>';
	}
	...
	private function th($str,$width=0,$hight=0,$css="")
	{
		...
		$tempstr .=$hight!=0?" hight=\"$hight\"":'';
		$tempstr .=$css!=0?" css=\"$css\"":'';
		$tempstr .=' style="text-align:center;BACKGROUND: url(../images/table_bg.png) #f4f4f4 repeat-x 0px 0px;">'.$str.'</th>';
		return $tempstr;
	}
