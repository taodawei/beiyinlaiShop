...
class shl_rss
{
	...
	function shl_rss($db,$cid,$num=9)
	{
		$this->db=$db;
		$this->cid = $cid;
		
			$this->menu_arr=!$this->cid 
							? array('id'=>'0','title'=>'','summary'=>'','type'=>'index','related_common'=>'','orderby'=>'0','level'=>0)
	...
	{
		return !$this->menu_arr['summary']? SITESUMMARY : ($this->menu_arr['summary'].' '.SITESUMMARY) ;
	...
	private function rss_header()
	{
	    ...
	    $this->xml.='  <description><![CDATA['.$this->get_summary().']]></description>';
		$this->xml.='  <generator>http://shenhoulong.com/?v=4.0</generator>';
	...
	private function model_case_info()
	{
		$arr=array(
			...
			'solutions'=>array(
				'SELECT a.id,a.title,b.content,a.author,a.dtTime FROM '.$this->tb_pix.'solutions a JOIN '.$this->tb_pix.'solutions_content b ON a.guid=b.guid WHERE channelId='.$this->cid.' ORDER BY a.id DESC LIMIT '.$this->num,
				'solutions_rss_format'
			),
			'calllist'=>array(
	...
	{	
		$item='<item>';
		$item.='	<title>'.$this->o->pageName.'</title>';
		$item.='	<link>'.$this->http.( !$this->rewrite ? ('/?p='.$this->cid.'&amp;mdtp='.(++$this->o->pageId)) :($this->get_menuName().(++$this->o->pageId))).'/</link>';
		$item.='	<description><![CDATA['.$this->filtered_chars($this->o->summary?$this->o->summary:$this->o->content).']]></description>';
	...
	{
		$item='<item>';
		$item.='	<title>'.$this->o->softwareName.'</title>';
		$item.= '	<link>'.$this->http.( !$this->rewrite ? ('/?p='.$this->cid.'&amp;a=view&amp;r='.$this->o->id) : ($this->get_menuName().'d'.$this->o->id.'.html') ).'</link>';
		$item.='	<description><![CDATA['.$this->filtered_chars($this->o->introduce).']]></description>';
		$item.='	<pubDate>'.$this->format_time($this->o->dtTime).'</pubDate>';
		$item.='</item>';
		return $item;
	}
	...
	{
		$item='<item>';
		$item.='	<title>'.$this->o->company.'</title>';
		$item.= '	<link>'.$this->http.( !$this->rewrite ? ('/?p='.$this->cid) : $this->get_menuName() ).'</link>';
	...
	{
		$item='<item>
				 <title>'.$this->o->linkerName.'</title>
					<link>'.$this->http.'/?p='.$this->cid.'&amp;r='.$this->o->id.'</link>
					<description><![CDATA['.$this->filtered_chars($this->o->summary).']]></description>
	...
	private function solutions_rss_format()
	{
		$item='<item>';
		$item.= '	<title>'.$this->o->title.'</title>';
		$item.= '	<link>'.$this->http.( !$this->rewrite ? ('/?p='.$this->cid.'&amp;a=view&amp;r='.$this->o->id) : ($this->get_menuName().'solutions_'.$this->o->id.'.html') ).'</link>';
		$item.= '	<description><![CDATA['.$this->filtered_chars($this->o->content).']]></description>';
		$item.='	<content:encoded><![CDATA['.$this->o->content.']]></content:encoded>';
		$item.= '	<pubDate>'.$this->format_time($this->o->dtTime).'</pubDate>';
		$item.= '</item>';
