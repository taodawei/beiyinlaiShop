 ...
 */
function build_form($order_ids,$print_name) {
	global $db;
	$comId = (int)$_SESSION[TB_PREFIX.'comId'];
	$fenbiao = getFenbiao($comId,20);
	$str = '';
	$fahuo_orders = $db->get_var("select fahuoIds from fahuo_pici$fenbiao where id=".$order_ids);//获取发货单id
	if($fahuo_orders){
		$orders=$db->get_results("select * from order_fahuo$fenbiao where id in (".$fahuo_orders.")");
		$db->query("update fahuo_pici$fenbiao set is_print=is_print+1 where id=".$order_ids);

		if($orders){
			foreach ($orders as $order) {
				$storeId = $db->get_var("select storeId from order_fahuo$fenbiao where id=".$order->id);//店铺id
