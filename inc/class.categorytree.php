...
class adminCategoryTree
{
	...
	function findChild($inputMenu)
	{
		...
		if($this->ordering){
			...
			echo '<tr style="background-color:#C5EAF5;height:20px;"><td><b><span>'.$printDeep.'<a href="./index.php?p='.$inputMenu->channelId.'&c='.$inputMenu->id.'&d='.strval(intval($inputMenu->deep)+1).'">'.$inputMenu->title.'</a></span></b></td></tr>';
			else
			echo '<tr style="background-color:#f2f2f2;height:20px"><td><b><span>'.$printDeep.'<a href="./index.php?p='.$inputMenu->channelId.'&c='.$inputMenu->id.'&d='.strval(intval($inputMenu->deep)+1).'">'.$inputMenu->title.'</a></span></b></td></tr>';
		}
