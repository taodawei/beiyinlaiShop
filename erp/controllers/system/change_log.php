...
function getList(){
	...
	$sql = "select * from kmd_change_log where status = 0 ";
...
// 		switch ($type){
// 			case 0:
// 				$sql.=" and (status=0 or print_type=1) ";
// 			break;
// 			case 1:
// 				$sql.=" and status=0";
// 			break;
// 			case 2:
// 				$sql.=" and status=1 and print_type=1";
// 			break;
// 			case 3:
// 				$sql.=" and status=3 and print_type=1";
// 			break;
// 		}
// 	}elseif($print_type==2){
// 		switch ($type){
...
// 	}
	if(!empty($keyword)){
	    $cardIds = $db->get_var("select group_concat(id) from kmd_change_card where card_no like '%$keyword%' or changeTitle like '%$keyword%' and userId > 0 ");
	    if(!$cardIds) $cardIds = 0;
		$sql.=" and (orderId like '%$keyword%' or shouhuo_json like '%$keyword%' or pdtInfo like '%$keyword%' or cardId in ($cardIds) )";
	}
	if(!empty($mendian)){
...
// 	if(!empty($fahuoTime)){
// 		$sql.=" and fahuoTime like '$fahuoTime%'";
// 	}
	if(!empty($mendian)){
...
function order_info_index(){
	...
	$fenbiao = getFenbiao($comId,20);
	$id = (int)$request['id'];
	$order = $db->get_row("select * from kmd_change_log where id=$id");

	if(empty($order))die("订单不存在！！");
	
// 	if($order->kuaidi_type == 0){
        $order->kuadi_company = '快递配送';
    // }
	...
	$user = $db->get_row("select u.* from users u  where u.id = $order->userId");
    // echo "select u.* from users u inner join order$fenbiao o on o.userId = u.id where o.orderId = '$orders'";
    // var_dump($user);
	//拼接字符串
	$str = '<div class="ddxx_jibenxinxi">';
	if($order->status==0){
// 		$str.='<div class="ddxx_jibenxinxi_1" id="order_operate">订单操作：<a href="javascript:" onclick="fahuo_show('.$id.');">发货</a>&nbsp;&nbsp;<a href="?s=fahuo&a=prints&id='.$id.'" target="_blank">打印</a>';
...
//批量修改发货时间
function order_pi_fahuoTime(){
	global $db,$request;
	$comId = (int)$_SESSION[TB_PREFIX.'comId'];
	$fenbiao = getFenbiao($comId,20);
	$ids = $request['ids'];
	$fahuoTime = $request['fahuo_time'];
	$db->query("update order_fahuo$fenbiao set fahuoTime='$fahuoTime' where id in($ids)");
	echo '{"code":1,"message":"操作成功"}';
	exit;
}
//批量社区发货
function order_pi_fahuo(){
	global $db,$request;
	$ids = $request['ids'];
	...
	//设置发货表订单状态
		if(!empty($ids)){
			...
			foreach ($ids_arr as $id) {
				$db->query("update order_fahuo$fenbiao set status=3,kuaidi_company='',kuaidi_title='".$request['kuaidi_company']."',kuaidi_order='".$request['kuaidi_order']."',rider_id=$rider_id,print_type=1 where id=$id");
				$orderIds = $db->get_var("select orderIds from order_fahuo$fenbiao where id=$id");
				if(!empty($orderIds)){
					$db->query("update order$fenbiao set status=3 where id in (".$orderIds.") and status in(2,3)");
...
function order_dianzi_info(){
	...
	$user = $db->get_row("select * from users where id = $order->userId");
	//拼接字符串
	$str = '<div class="ddxx_jibenxinxi">';
	/*if($order->status>-1){
...
function order_xiangqing_index(){
	...
	$fenbiao = getFenbiao($comId,20);
	$id = (int)$request['id'];
	$pdtInfo = $db->get_var("select pdtInfo from kmd_change_log where id=$id");

	$products = array();
	if(!empty($pdtInfo)){
		$pdtInfo = json_decode($pdtInfo, true);
	    $inventory = $db->get_row("select * from demo_product_inventory where id =". $pdtInfo['id']);
...
function order_fahuo(){
	...
	//设置发货表订单状态
	if($id>0){
		$nowStatus = $db->get_var("select status from kmd_change_log where id=$id");
		if($nowStatus!=0){
			echo '{"code":0,"message":"当前发货单状态不能再进行普通发货"}';
			exit;
		}
		
		$source = $db->get_var("select express_type from demo_shezhi where comId = $comId ");
        $kuaidis = $db->get_results("select  name kuaidi_company,express_no kuaidi_type from express_company  where source = $source order by id ");
        if($source == 3){
            ...
            $kuaidi_company = $db->get_var("select express_no from express_company where source = $source and name = '".$request['kuaidi_company']."' ");
        }
		
		$db->query("update kmd_change_log set status=1,kuaidi_company='".$kuaidi_company."',kuaidi_title='".$request['kuaidi_company']."',kuaidi_order='".$request['kuaidi_order']."',fahuoTime='".date('Y-m-d H;i:s')."' where id=$id");
...
function get_daoru_list(){
	...
	$sql = "select * from fahuo_pici$fenbiao where comId=$comId and type=2";
...
function daoru_order(){
	...
	if(!empty($jilus)){
		...
		foreach ($jilus as $jilu){
			$kuaidi_company = '未知';
			$order = $db->get_row("select * from kmd_change_log where orderId='".$jilu[0]."' and status=0 limit 1");
			if(!empty($order->id)){
			    if($source == 3){
			        foreach ($arr as $keys => $values) {
    					if(strstr($keys, $jilu[6]) || strstr($jilu[6] , $keys)){
    						//$pandianJsonData = str_replace("'","\'",$pandianJsonData);
    						$kuaidi_company = $values;
    						break;
    					}
	                ...
	                $kuaidi_company = $db->get_var("select express_no from express_company where source = $source and name = '".$jilu[6]."' ");
			    }
				
				$success_num ++;
				$fahuoIds .= $order->id.',';
			
				$db->query("update kmd_change_log set status=1,kuaidi_company='".$kuaidi_company."',kuaidi_title='".$jilu[6]."',kuaidi_order='".$jilu[7]."' ,fahuoTime='".date('Y-m-d H:i:s')."' where orderId='".$jilu[0]."' limit 1");//设置发货单完成
...
function fahuo(){
	...
	if(!empty($request['ids']) && !empty($request['kuaidi_id'])){
		...
		$yushouId = (int)$request['yushouId'];
		$db->query("insert into fahuo_pici$fenbiao(comId,type,orderId,fahuoIds,num,dtTime,storeId,kuaidiniao_id,yushouId) values ($comId,2,'".$orderId."','',".$num.",'".date('Y-m-d H:i:s')."',$pici_storeId,".$request['kuaidi_id'].",$yushouId)");
		...
		//请求url，正式环境地址：http://api.kdniao.com/api/Eorderservice    测试环境地址：http://sandboxapi.kdniao.com:8080/kdniaosandbox/gateway/exterfaceInvoke.json
		...
		//file_put_contents('222.txt',json_encode($orders, JSON_UNESCAPED_UNICODE));
		if(!empty($orders)){
			...
			foreach ($orders as $order) {
				...
				if(!empty($kuaidiniao->SendSite)){
					$eorder["SendSite"] =	$kuaidiniao->SendSite;
				}
							...
							if(!empty($orderIds)){
								foreach ($orderIds as $k => $v) {
									...
									if($_SESSION['tongbu_zong']==1){
										$update = array();
										$update['status'] = 3;
										$update['fahuo_json'] = '{"company":"'.$request['kuaidi_company'].'","entitle":"'.$request['kuaidi_title'].'","orderId":"'.$result['Order']["LogisticCode"].'"}';
										tongbu_order($v,$update);
									}
								}
							}
						}
...
function getfhsuccess(){
	...
	$sql = "select * from kmd_change_log where (status = 1 or status = 4) ";
	...
	if(!empty($keyword)){
		$sql.=" and (orderId like '%$keyword%' or kuaidi_order like '%$keyword%'  or shouhuo_json like '%$keyword%' or pdtInfo like '%$keyword%')";
	}
	...
	if(!empty($jilus)){
		foreach ($jilus as $i=>$j) {
			...
			switch ($j->status) {
				...
				break;
				case 1:
					$status = '<span style="color:#ff3333;">已发货</span>';
				break;
			} 
			...
			$j->address = $shuohuo_json['address'].$shuohuo_json['详细地址'];
			$j->shouhuo = $shuohuo_json['name'];
			$j->tel = $shuohuo_json['mobile'];
			$j->beizhu = '<span onmouseover="tips(this,\''.$j->beizhu.'\',1);" onmouseout="hideTips()">'.sys_substr(strip_tags($j->beizhu),20,true).'</span>';
			$j->mendian = $db->get_var("select title from demo_shequ where id=$j->mendianId");
			$product_array = array();
			if(!empty($j->pdtInfo))$product_array = json_decode($j->pdtInfo);
			$j->pdt_info = '';
			if(!empty($product_array)){
			    $productArr[] = $product_array;
