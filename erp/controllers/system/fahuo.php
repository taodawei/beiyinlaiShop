...
function getList(){
	...
	$sql = "select id,orderId,comId,mendianId,type,status,dtTime,remark,shuohuo_json,product_title,weight,addressId,is_hebing from order_fahuo$fenbiao where comId=$comId and status<>-1   and  status <> 3 ";
	...
	switch($scene){
	    case 1:
	        $sql .= " and find_in_set(1, pay_types) ";
	        break;
        case 2:
	        $sql .= " and find_in_set(2, pay_types) ";
	        break;
        case 3:
	        $sql .= " and find_in_set(3, pay_types) ";
	        break;      
	}
...
function order_info_index(){
	...
	$user = $db->get_row("select u.* from users u inner join order$fenbiao o on o.userId = u.id where o.orderId = '$orders'");
	...
	$str.='<div class="dianzimiandanxx_jibenxinxi_02">
                	<div class="dianzimiandanxx_jibenxinxi_02_up">
                    	发货单信息'.(($order->status==0||$order->status==1)?'<a href="javascript:" style="color:red;margin-left:20px;" onclick="xiugai_shouhuo(\''.$order->id.'\',\''.$shuohuo_json["收件人"].'\',\''.$shuohuo_json["手机号"].'\',\''.$shuohuo_json["所在地区"].'\',\''.$shuohuo_json["详细地址"].'\')">修改收货信息</a><a href="javascript:" style="color:red;margin-left:20px;display:none;" onclick="fahuo_tuikuan('.$order->id.');">订单退款</a>':'').'
                    </div>
...
function order_fahuo(){
	...
	//设置发货表订单状态
	if($id>0){
		$nowStatus = $db->get_var("select status from order_fahuo$fenbiao where id=$id");
		if($nowStatus!=0){
		...
		$db->query("update order_fahuo$fenbiao set status=3,kuaidi_company='".$kuaidi_company."',kuaidi_title='".$request['kuaidi_company']."',kuaidi_order='".$request['kuaidi_order']."',fahuoTime='".date('Y-m-d H;i:s')."' where id=$id");
...
function daoru_order(){
	...
	if(!empty($jilus)){
		...
		foreach ($jilus as $jilu){
			$kuaidi_company = '';
			$order = $db->get_row("select id,status,storeId,mendianId from order_fahuo$fenbiao where orderId='".$jilu[0]."' and status=1 limit 1");
			if(!empty($order->id)){
			    if($source == 3){
			        foreach ($arr as $keys => $values) {
    					if(strstr($keys, $jilu[7]) || strstr($jilu[7] , $keys)){
    						//$pandianJsonData = str_replace("'","\'",$pandianJsonData);
    						$kuaidi_company = $values;
    						break;
    					}
	                ...
	                $kuaidi_company = $db->get_var("select express_no from express_company where source = $source and name = '".$jilu[7]."' ");
			    }
				...
				$db->query("update order_fahuo$fenbiao set status=3,kuaidi_company='".$kuaidi_company."',kuaidi_title='".$jilu[7]."',kuaidi_order='".$jilu[8]."' ,fahuoTime='".date('Y-m-d H:i:s')."' where orderId='".$jilu[0]."' limit 1");//设置发货单完成
				$orderIds = explode(',',$orderIds);
				//减去库存，微信模板消息
