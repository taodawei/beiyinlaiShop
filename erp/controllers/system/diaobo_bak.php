...
function add(){
	...
	if($request['tijiao']==1){
		...
		$dtTime = date("Y-m-d",strtotime($request['dtTime']));
		$type = 3;
		...
		if(!empty($request['inventoryNum'])){
			...
			foreach ($request['inventoryNum'] as $key => $num){
				...
				$kucun = $k->kucun;
				$kucun-=$num;
				$kucun1 = $db->get_var("select kucun from demo_kucun where inventoryId=$inventoryId and storeId=$store1Id limit 1");
				$kucun1+=$num;
				...
				$lastJilu1 = $db->get_row("select kucun,zongchengben from demo_kucun_jiludetail$fenbiao where inventoryId=$inventoryId and storeId=$store1Id and status=1 order by id desc limit 1");
				if(empty($lastJilu)){
				...
				if(empty($lastJilu1)){
					$lastJilu1->zongchengben = 0;
					$lastJilu1->kucun = 0;
				}
				$zongchengben1 = $lastJilu1->zongchengben+$rukuChengben;
				$zongNum = $kucun1;
				$chengben = getXiaoshu($zongchengben1/$zongNum,4);
				if($status==1){
					$db->query("update demo_kucun set kucun=kucun-$num where inventoryId=$inventoryId and storeId=$storeId limit 1");
					$db->query("update demo_kucun set kucun=kucun+$num,chengben='$chengben' where inventoryId=$inventoryId and storeId=$store1Id limit 1");
				}else{
				...
				$rukuSql1.=",($comId,$jiluId,$inventoryId,$productId,'$pdtInfo',$store1Id,'$store1Name','$num',$status,'$kucun1','$beizhu',1,'调拨入库','$dtTime','$units','$rukuChengben','$zongchengben1')";
			}
...
function getJilus(){
	...
	$sql="select id,orderId,dtTime,storeName,store1Name,type_info,username,status,shenheTime from demo_kucun_jilu$fenbiao where comId=$comId and type=3 ";
	if(!empty($storeIds)){
	...
	$count = $db->get_var(str_replace('id,orderId,dtTime,storeName,store1Name,type_info,username,status,shenheTime','count(*)',$sql));
