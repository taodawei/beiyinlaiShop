...
function xiajia(){
	...
	layer.confirm('确定要取消关联选中的'+num+'个产品吗？', {
	  btn: ['确定','取消'],
	},function(){
		layer.closeAll();
		layer.load();
		ajaxpost=$.ajax({
			type: "POST",
			url: "?m=system&s=supplier&a=qxguanlian",
			data: "&supplierId="+supplierId+"&ids="+ids,
...
function delAll(){
	var ids = $("#selectedIds").val();
	var supplierId = $("#supplierId").val();
	layer.confirm('确定要取消关联该供应商所有供应产品吗？', {
	  btn: ['确定','取消'],
	},function(){
		layer.closeAll();
		layer.load();
		ajaxpost=$.ajax({
			type: "POST",
			url: "?m=system&s=supplier&a=qxguanlianAll",
