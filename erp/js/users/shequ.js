...
function bohui(){
	...
	$.ajax({
		type: "POST",
		url: "?m=system&s=users&a=bohuiShequ",
		data: "id="+jiluId,
...
function shenhe(){
	...
	$.ajax({
		type: "POST",
		url: "?m=system&s=users&a=sheheShequ",
		data: "id="+jiluId,
		dataType:"json",timeout : 10000,
		success: function(resdata){
			layer.closeAll();
...
function edit(){
	jiluId = getPdtId();
	location.href="?s=users&a=add_shequ&id="+jiluId;
}
function view(){
	jiluId = getPdtId();
	var keyword = $("#keyword").val();
	...
	var order2 = $("#order2").val();
	var status = $("#status").val();
	var url = "?m=system&s=users&a=shequ&status="+status+"&keyword="+keyword+"&page="+page+"&order1="+order1+"&order2="+order2;
	url = encodeURIComponent(url);
	location.href="?s=users&a=view_shequ&id="+jiluId+"&returnurl="+url;
}