...
function guigeTable(){
	...
	$.ajax({
		type: "POST",
		url: "?m=system&s=product&a=getPricesTabel&if_create=1",
		data: data,
...
function checkDinghuoNum(dom,type){
	var min = 0;
	var max = 0;
	if(type==1){
		min = parseFloat($(dom).val());
		max = parseFloat($(dom).parent().next().find("input").eq(0).val());
	}else{
		max = parseFloat($(dom).val());
		min = parseFloat($(dom).parent().prev().find("input").eq(0).val());
	}
	if(min>0&&max>0&&min>max){
		layer.msg('起订量不能小于限订量！',function(){});
		$(dom).addClass('layui-form-danger').val('0').focus();
	}
}