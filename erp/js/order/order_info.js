...
function order_show(zindex){
	...
	var tuihuanId = parseInt($(nowtr).find("td[data-field='tuihuanId'] div").text());
	...
	if(id==dataId){
		...
		$("#dqddxiangqing").attr("data-tuihuanId",tuihuanId);
...
function order_info_index(force){
	...
	if($("#orderInfoCont1 .loading").length>0||force==1){
		...
		ajaxpost=$.ajax({
			type: "POST",
			url: "?s=order&a=order_info_index",
			data: "id="+orderId,
		    dataType : "text",
		    timeout : 20000,
...
function order_error_index(force){
	...
	if($("#orderInfoCont2 .loading").length>0||force==1){
		...
		ajaxpost=$.ajax({
			type: "POST",
			url: "?s=order&a=order_error_index",
			data: "id="+orderId,
		    dataType : "text",
		    timeout : 20000,
			success: function(data) {
...
function order_tuihuan_index(force){
	var orderId = parseInt($("#dqddxiangqing").attr("data-id"));
		var tuihuanId = parseInt($("#dqddxiangqing").attr("data-tuihuanId"));
...
function add_error_beizhu(errorId,is_done){
	...
	$.ajax({
		type: "POST",
		url: "?s=order&a=add_error_beizhu",
...
function add_order_beizhu(id){
	...
	$.ajax({
		type: "POST",
		url: "?s=order&a=add_order_beizhu",
...
function order_edit(orderId){
	...
	ajaxpost=$.ajax({
		type: "POST",
		url: "?s=order&a=order_info_getedit",
		data: "id="+orderId,
		dataType : "json",
		timeout : 20000,
		success: function(resdata) {
...
// 	}
	layer.open({
		...
		,yes: function(index, layero){
			...
			ajaxpost=$.ajax({
			type: "POST",
			url: "?s=order&a=shenhe_tuihuan",
			data: "&tuihuanId="+tuihuanId+"&tk_price="+beizhu,
		...
		,content: '<div class="spxx_shanchu_tanchu layui-form" style="display: block;">'+
		'<div class="spxx_shanchu_tanchu_01">'+
		'<div class="spxx_shanchu_tanchu_01_left">'+tishi+'</div>'+
		'<div class="spxx_shanchu_tanchu_01_right">'+
		'<a href="javascript:layer.closeAll();"><img src="images/biao_47.png"></a>'+
		'</div>'+
		'<div class="clearBoth"></div>'+
		'</div>'+
		'<div class="spxx_shanchu_tanchu_02" style="height:220px;padding:0px;margin-top:10px;">'+
		'<textarea id="e_beizhu" cols="30" rows="10" class="layui-textarea" placeholder="请输入退款金额">'+price+'</textarea>'+
		'</div>'+
		'</div>'
	});
...
function tuihuan_wancheng(){
	...
	layer.confirm('确定要将该订单设置为已退款吗？', {
	  btn: ['确定','取消'],
		...
		ajaxpost=$.ajax({
			type: "POST",
			url: "?s=order&a=wancheng_tuihuan",
			data: "tuihuanId="+tuihuanId,
			dataType:"json",timeout : 10000,
			success: function(resdata){
				layer.closeAll('loading');
...
function service_edit(orderId,serviceId,reload){
	...
	$.ajax({
		type: "POST",
		url: "?s=order&a=order_edit_service",
...
function tuihuan_fahuo(id){
	layer.open({
		...
		,btn: ['确定', '取消']
		,yes: function(index, layero){
			var company = $("#e_kuaidi_company").val();
			var orderId = $("#e_kuaidi_order").val();
			layer.load();
			$.ajax({
				type: "POST",
				url: "?s=order&a=tuihuan_fahuo&status=1",
				data: "id="+id+"&company="+company+"&orderId="+orderId,
				dataType:'json',timeout:30000,
				success: function(resdata){
					layer.closeAll();
					if(resdata.code==0){
		...
		,content: '<div class="spxx_shanchu_tanchu layui-form" style="display: block;">'+
		'<div class="spxx_shanchu_tanchu_01">'+
		'<div class="spxx_shanchu_tanchu_01_left">输入发货信息</div>'+
		'<div class="spxx_shanchu_tanchu_01_right">'+
		'<a href="javascript:layer.closeAll();"><img src="images/biao_47.png"></a>'+
		'</div>'+
		'<div class="clearBoth"></div>'+
		'</div>'+
		'<div class="spxx_shanchu_tanchu_02" style="height:90px;padding:0px;margin-top:10px;line-height:40px;">'+
		'快递公司：<input type="text" id="e_kuaidi_company" style="width:250px;height:27px;"/><br>物流单号：<input type="text" id="e_kuaidi_order" style="width:250px;height:27px;"/><br>'+
		'</div>'+
		'</div>'
	});
}
function order_kaipiao(id,type,url){
	if(type==1){
		layer.open({
			type: 1
			,title: false
			...
			,btn: ['确定', '取消']
			,yes: function(index, layero){
				var url = $.trim($("#fapiao_edit_url").val());
				if(url==''){
					layer.msg('发票地址不能为空',{icon:5});
					return false;
				}
				layer.load();
				$.ajax({
					type: "POST",
					url: "?s=order&a=fapiao_kaipiao",
			...
			,content: '<div class="spxx_shanchu_tanchu layui-form" style="display: block;">'+
			'<div class="spxx_shanchu_tanchu_01">'+
			'<div class="spxx_shanchu_tanchu_01_left">输入电子发票地址</div>'+
			'<div class="spxx_shanchu_tanchu_01_right">'+
			'<a href="javascript:layer.closeAll();"><img src="images/biao_47.png"></a>'+
			'</div>'+
			'<div class="clearBoth"></div>'+
			'</div>'+
			'<div class="spxx_shanchu_tanchu_02" style="height:90px;padding:0px;margin-top:10px;line-height:40px;">'+
			'电子发票地址：<input type="text" id="fapiao_edit_url" value="'+url+'" style="width:250px;height:27px;"/><br>'+
			'</div>'+
			'</div>'
		});
	}else{
		layer.open({
			...
			,content: '<div class="spxx_shanchu_tanchu layui-form" style="display: block;">'+
			'<div class="spxx_shanchu_tanchu_01">'+
			'<div class="spxx_shanchu_tanchu_01_left">输入发票快递信息</div>'+
			'<div class="spxx_shanchu_tanchu_01_right">'+
			'<a href="javascript:layer.closeAll();"><img src="images/biao_47.png"></a>'+
			'</div>'+
			'<div class="clearBoth"></div>'+
			'</div>'+
			'<div class="spxx_shanchu_tanchu_02" style="height:90px;padding:0px;margin-top:10px;line-height:40px;">'+
			'发票快递信息：<input type="text" id="fapiao_edit_url" value="'+url+'" placeholder="不填表示发票随商品一同寄出" style="width:250px;height:27px;"/><br>'+
			'</div>'+
			'</div>'
		});
	}
