...
function hexiao(){
	layer.open({
		...
		,yes: function(index, layero){
			...
			if(code==''){
				layer.msg("请输入核销码",function(){});
				return false;
			}
			layer.load();
			$.ajax({
				type: "POST",
				url: "?s=order1&a=hexiao",
				data: "code="+code,
				dataType:'json',timeout:10000,
				success: function(resdata){
					layer.closeAll('loading');
		...
		,content: '<div class="spxx_shanchu_tanchu layui-form" style="display: block;">'+
		'<div class="spxx_shanchu_tanchu_01">'+
		'<div class="spxx_shanchu_tanchu_01_left">核销</div>'+
		'<div class="spxx_shanchu_tanchu_01_right">'+
		'<a href="javascript:layer.closeAll();"><img src="images/biao_47.png"></a>'+
		'</div>'+
		'<div class="clearBoth"></div>'+
		'</div>'+
		'<div class="spxx_shanchu_tanchu_02" style="height:60px;padding:0px;margin-top:10px;">'+
		'<input id="code" type="text" class="layui-input" placeholder="请输入核销码" >'+
		'</div>'+
		'</div>'
	});
}