...
function render_channel_tags(channelId,productId){
	layer.load();
	$.ajax({
		type: "POST",
		url: "?s=product&a=get_channel_tags",
		data: "channelId="+channelId+"&productId="+productId,
		dataType:"json",timeout : 8000,
		success: function(resdata){
			layer.closeAll();
			var html = '';
			if(resdata.items.length>0){
				...
				$.each(resdata.items,function(key,val){
					if(val.ziTags!=null){
						html+='<tr><td valign="top" align="right" style="white-space:nowrap">'+val.title+'：</td><td style="padding-left:10px">';
						$.each(val.ziTags,function(key1,val1){
							html+='<input type="checkbox" name="tags[]" lay-skin="primary" value="'+val1.id+'" '+(val1.checked==1?'checked="true"':'')+' title="'+val1.title+'">';
						});
						html+='</td></tr>';
					}
				});
...
function bohui(productId){
	layer.open({
		...
		,yes: function(index, layero){
			...
			$.ajax({
				type: "POST",
				url: "?m=system&s=pdts&a=bohui",
				data: "productId="+productId+"&cont="+beizhu,
