	...
	$("#zanting_fahuo").click(function(){
		layer.confirm('确定要将选中发货单暂停吗？', {
		  btn: ['确定','取消'],
			...
			ajaxpost=$.ajax({
				type: "POST",
				url: "?s=pdts_fahuo&a=zanting_fahuo",
				data: "&ids="+ids,
...
function selectStore(storeId,title){
	...
	$.ajax({
		type: "POST",
		url: "?s=pdts_fahuo&a=get_miandans",
...
function print_fahuo(){
	...
	$.ajax({
		type: "POST",
		url: "?s=pdts_fahuo&a=fahuo",
		data: "ids="+ids+"&kuaidi_company="+kuaidi_company+"&kuaidi_title="+kuaidi_title+"&kuaidi_id="+kuaidi_id,
