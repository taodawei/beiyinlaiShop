...
function order_dianzi_info(force){
	...
	if($("#orderInfoCont1 .loading").length>0||force==1){
		...
		ajaxpost=$.ajax({
			type: "POST",
			url: "?s=pdts_fahuo&a=order_fahuo_info",
			data: "id="+orderId+"&keyword="+keyword,
		    dataType : "text",
		    timeout : 20000,
...
function order_xiangqing_index(force){
	...
	if($("#orderInfoCont2 .loading").length>0||force==1){
		...
		ajaxpost=$.ajax({
			type: "POST",
			url: "?s=pdts_fahuo&a=order_xiangqing_index",
			data: "id="+orderId,
		    dataType : "text",
		    timeout : 20000,
			success: function(data) {
...
//发货订单
function fahuo_order_index(force){
	var orderId = parseInt($("#dqddxiangqing").attr("data-id"));
	var keyword = $("#keyword").val();
	if($("#orderInfoCont2 .loading").length>0||force==1){
		if($("#orderInfoCont2 .loading").length==0){
		...
		ajaxpost=$.ajax({
			type: "POST",
			url: "?s=pdts_fahuo&a=fahuo_order_index",
			data: "id="+orderId+"&keyword="+keyword,
		    dataType : "text",
		    timeout : 20000,
			success: function(data) {
...
function order_jilu_index(force){
	...
	if($("#orderInfoCont5 .loading").length>0||force==1){
		...
		ajaxpost=$.ajax({
			type: "POST",
			url: "?s=pdts_fahuo&a=order_jilu_index",
			data: "id="+orderId,
		    dataType : "text",
		    timeout : 20000,
			success: function(data) {
