...
function order_info_index(force){
	...
	if($("#orderInfoCont1 .loading").length>0||force==1){
		...
		ajaxpost=$.ajax({
			type: "POST",
			url: "?s=fahuo&a=order_info_index",
			data: "id="+orderId,
		    dataType : "text",
		    timeout : 20000,
...
function order_show_card(zindex){
	if(typeof(zindex)=='undefined'){
		var zindex = $("#nowIndex").val();
	}else{
		$("#nowIndex").val(zindex);
...
//获取订单基础信息，force是否强制刷新 0否1是
function order_info_card(force){
	var orderId = parseInt($("#dqddxiangqing").attr("data-id"));
	if($("#orderInfoCont1 .loading").length>0||force==1){
		if($("#orderInfoCont1 .loading").length==0){
			$("#orderInfoCont1").html('<div class="loading"><img src="images/loading.gif"></div>');
		...
		ajaxpost=$.ajax({
			type: "POST",
			url: "?s=change&a=order_info_index",
			data: "id="+orderId,
		    dataType : "text",
		    timeout : 20000,
			success: function(data) {
...
function order_xiangqing_index(force){
	...
	if($("#orderInfoCont2 .loading").length>0||force==1){
		...
		ajaxpost=$.ajax({
			type: "POST",
			url: "?s=fahuo&a=order_xiangqing_index",
			data: "id="+orderId,
		    dataType : "text",
		    timeout : 20000,
			success: function(data) {
