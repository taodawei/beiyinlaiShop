...
function selectRow(id,inventoryId,sn,title,key_vals,productId,units,kucun,price,num){
	...
	'<td bgcolor="#ffffff" class="sprukuadd_03_tt" align="center" valign="middle">'+num+'</td>'+
	'<td bgcolor="#ffffff" width="175" class="sprukuadd_03_tt" align="center" valign="middle">'+
		'<input type="text" lay-verify="required|number|kucun" name="inventoryNum['+id+']" max="'+kucun+'" value="'+kucun+'" onmouseenter="tips(this,\'最多可退'+kucun+'\',1)" onmouseout="hideTips();" onchange="renderPrice('+id+');" class="sprukuadd_03_tt_input">'+
...
function getPdtInfo(id,keyword){
	...
	$.ajax({
		type: "POST",
		url: "/erp_service.php?action=getTuihuoList&id="+id+"&storeId="+storeId+"&caigouId="+caigouId,
...
function readerAllItems(){
	...
	if(caigouId!=""&&storeId!=""){
	    ...
	    $.ajax({
			type: "POST",
			url: "/erp_service.php?action=getAllTuihuos&storeId="+storeId+"&caigouId="+caigouId,
			data: "",
			dataType:'json',timeout : 30000,
			success: function(resdata){
				$("#dataTable").attr("rows",resdata.count);
				$("#rowTrHeji").before(resdata.str);
				addRow();
	    		readerTr();
	    		renderAllPrice();
	    		layer.closeAll();
