...
function edit_channel(id,pid,title, en_title, imgurl,backimg,is_hot = 0, type=0, remark =''){
	...
	ajaxpost=$.ajax({
		type: "POST",
		url: "/erp_service.php?action=get_skill_channels",
		data: "&id="+id+"&pid="+pid,
		...
		success: function(resdata) {
			...
			layer.open({
		        ...
		        ,content: '<div class="spxx_shanchu_tanchu" style="display: block;height: 300px;">'+
					'<form action="?m=system&s=skill_channel&a=addProductChannel&id='+id+'" method="post" id="channelForm"><div class="spxx_shanchu_tanchu_01">'+
...
function delChannel(id){
	...
	ajaxpost=$.ajax({
		type: "POST",
		url: "?m=system&s=skill_channel&a=delChannel",
		data: "&id="+id,
		dataType:"json",
		timeout : 20000,
		success: function(resdata) {
