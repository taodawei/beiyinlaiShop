...
function check_tuijian(dom){
	var username = $(dom).val();
	if(username!=''){
		$.ajax({
			type: "POST",
			url: "?s=mendian&a=check_tuijianren",
			data: "username="+username,
			dataType:"json",timeout : 8000,
			success: function(resdata){
				$("#tuijianren_name").text(resdata.message);
				if(resdata.code==0){
...
function checkPassForm(id){
	...
	$.ajax({
		type: "POST",
		url: "?m=system&s=mendian&a=updatePass&id="+id,
