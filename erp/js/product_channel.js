...
function edit_channel(id,pid,title,imgurl,backimg,is_hot = 0, type=0, remark =''){
    
    window.location.href="?m=system&s=product_channel&a=addProductChannel&id="+id+"&pid="+pid; //2.当前页面打开URL页面
	...
	ajaxpost=$.ajax({
		type: "POST",
		url: "/erp_service.php?action=get_product_channels",
		data: "&id="+id+"&pid="+pid,
		...
		success: function(resdata) {
			...
			layer.open({
		        ...
		        ,content: '<div class="spxx_shanchu_tanchu" style="display: block;height: 400px;">'+
					'<form action="?m=system&s=product_channel&a=addProductChannel&id='+id+'" method="post" id="channelForm"><div class="spxx_shanchu_tanchu_01">'+
				...
				// 			'<select name="type">' +selectType+ '</select><br>'+
							'<span>*</span> 热门推荐 '+
							'<select name="is_hot">' +selectHot+ '</select><br>'+
...
function delChannel(id){
	...
	ajaxpost=$.ajax({
		type: "POST",
		url: "?m=system&s=product_channel&a=delChannel",
		data: "&id="+id,
		dataType:"json",
		timeout : 20000,
		success: function(resdata) {
