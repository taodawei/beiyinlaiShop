...
$allRows = array(
    ...
    "card_no"=>array("title"=>"卡号","rowCode"=>"{field:'card_no',title:'卡号',width:150,align:'center'}"),
    "card_pass"=>array("title"=>"密码","rowCode"=>"{field:'card_pass',title:'密码',width:200,align:'center'}"),
    "open_info"=>array("title"=>"是否开通","rowCode"=>"{field:'open_info',title:'是否开通',width:134,align:'center'}"),
    
    "openTime"=>array("title"=>"开通时间","rowCode"=>"{field:'openTime',title:'开通时间',width:160,align:'center'}"),
    
    "userInfo"=>array("title"=>"用户昵称","rowCode"=>"{field:'userInfo',title:'用户昵称',width:180,align:'center'}"),
    "changeTitle"=>array("title"=>"卡标题","rowCode"=>"{field:'changeTitle',title:'卡标题',width:140,align:'center'}"),
    "change_time"=>array("title"=>"可兑次数","rowCode"=>"{field:'change_time',title:'可兑次数',width:120,align:'center'}"),
    "had_time"=>array("title"=>"已兑次数","rowCode"=>"{field:'had_time',title:'已兑次数',width:120,align:'center'}"),
    ...
    "status_info"=>array("title"=>"状态","rowCode"=>"{field:'status_info',title:'状态',width:134,align:'center'}"),
    "dtTime"=>array("title"=>"生成时间","rowCode"=>"{field:'dtTime',title:'生成时间',width:220,align:'center'}"),
...
$id = $request['id'];
$title = $db->get_var("select title from kmd_change where id = $id ");
$title = $title.' 兑换卡列表';
        ...
        <div class="splist">
            <div class="splist_up" style="height:118px;">
                <div class="splist_up_addtab">
                    <ul>
                        <li>
                            <a href="?s=change&a=card&id=<?=$id?>&scene=<?=$scene?>" <? if(empty($type)){?>class="splist_up_addtab_on"<? }?>>全部</a>
                        </li>
                        <li>
                            <a href="?s=change&a=card&id=<?=$id?>&type=1&scene=<?=$scene?>" <? if($type==1){?>class="splist_up_addtab_on"<? }?>>未兑换</a>
                        </li>
                        <li>
                            <a href="?s=change&a=card&id=<?=$id?>&type=2&scene=<?=$scene?>" <? if($type==2){?>class="splist_up_addtab_on"<? }?>>已兑换</a>
                        </li>
                    ...
                    <div class="splist_up_01_right">
                        <div class="splist_up_01_right_1">
                            <div class="splist_up_01_right_1_left">
                                <input type="text" id="keyword" value="<?=$keyword?>" placeholder="卡号/用户昵称/用户手机号"/>
                            </div>
                        ...
                        <div class="splist_up_01_right_3">
                            <a href="../upload/excel_template/changeCard.xlsx" class="splist_add" style="background-color:green;">下载模板</a>
						    ...
						    <? //chekurl($arr,'<a href="?m=system&s=change&a=batchAdd&changeId='.$id.'" class="splist_add">批量新增</a>') ?>
						    
						    <? chekurl($arr,'<a href="javascript:;" _href="?m=system&s=change&a=importCards" id="uploadFile" class="splist_add" style="background-color:red;">批量导入</a>') ?>
...
layui.use(['upload','form'], function(){
	...
	upload.render({
	    ...
	    ,done: function(res){
	      ...
	      $.ajax({
			type:"post",
			url:"?m=system&s=change&a=importCards&changeId=<?=$id?>",
    ...
    layui.use(['laydate', 'laypage','table','form'], function(){
        ...
        productListTalbe = table.render({
            elem: '#product_list'
            ,height: "full-200"
            ,url: '?m=system&s=change&a=getCardList&print_type=1&changeId=<?=$id?>'
            ,page: {curr:<?=$page?>}
            ,limit:<?=$limit?>
            ,cols: [[<?=$rowsJS?>]]
            ,where:{
    ...
    $("#zuofei").click(function(){
		layer.confirm('确定要将选中兑换卡关闭吗？', {
		  btn: ['确定','取消'],
			...
			ajaxpost=$.ajax({
				type: "POST",
				url: "?s=change&a=zuofei&is_open=0",
				data: "&ids="+ids,
				dataType:"json",timeout : 10000,
				success: function(resdata){
					layer.closeAll();
    ...
    $("#kaitong").click(function(){
		layer.confirm('确定要将选中兑换卡开通吗？', {
		  btn: ['确定','取消'],
			...
			ajaxpost=$.ajax({
				type: "POST",
				url: "?s=change&a=zuofei&is_open=1",
				data: "&ids="+ids,
				dataType:"json",timeout : 10000,
				success: function(resdata){
					layer.closeAll();
