...
$allRows = array(
				"logo"=>array("title"=>"图片","rowCode"=>"{field:'logo',title:'图片',width:80}"),
				"title"=>array("title"=>"商品名称","rowCode"=>"{field:'title',title:'商品名称',width:150}"),
				"key_vals"=>array("title"=>"规格","rowCode"=>"{field:'key_vals',title:'规格',width:150}"),
				"sn"=>array("title"=>"编号","rowCode"=>"{field:'sn',title:'编号',width:150}"),
				"startTime"=>array("title"=>"生效时间","rowCode"=>"{field:'startTime',title:'生效时间',width:250}"),
				"endTime"=>array("title"=>"失效时间","rowCode"=>"{field:'endTime',title:'失效时间',width:250}")
			);
...
$id = $request['id'];
$inventory = $db->get_row("select * from kmd_change where id = $id ");
...
$channels = array();
// $channels = $db->get_results("select id,title from demo_product_pinlei where comId=$comId and parentId = 0 order by ordering desc,id asc");
$tagsarry = array();
...
<head>
	...
	<style>
		...
		td[data-field="logo"] img{border:#abd3e7 1px solid}
...
<body>
	<div class="right_up">
		<img src="images/biao_19.png"/> <?=$inventory->title?>-可兑商品列表
	</div>
	<div class="right_down" style="padding-bottom:0px;">
					...
					<div class="splist_up_01_left">
						<div class="splist_up_01_left_01" style="display:none;">
							<div class="splist_up_01_left_01_up">
								<span>全部品类</span> <img src="images/biao_20.png"/>
							</div>
							<div class="splist_up_01_left_01_down">
								<ul style="border-left:0px" id="ziChannels1">
									<li class="allsort_01">
										<a href="javascript:selectChannel(0,'全部品类');">全部品类</a>
									</li>
									<? if(!empty($channels)){
     ...
     <!--               	<input type="checkbox" name="hebing" lay-filter="hebing" lay-skin="primary" checked="true" title="将可兑商品合并">-->
     <!--               </div>-->
					<div class="splist_up_01_right">	
						<div class="splist_up_01_right_1">
							<div class="splist_up_01_right_1_left">
								<input type="text" id="keyword" value="<?=$keyword?>" placeholder="请输入可兑商品名称"/>
							</div>
											...
											<ul>
												<li>
													...
													<div class="gaojisousuo_right">
														<input type="text" name="super_keyword" class="gaojisousuo_right_input" placeholder="请输入可兑商品名称/编码/规格/关键字/条形码"/>
													</div>
													<div class="gaojisousuo_left">
														可兑商品分类
													</div>
						...
						<div class="splist_up_01_right_3">
						    <? chekurl($arr,'<a href="?m=system&s=change&a=addProduct&changeId='.$id.'" class="splist_add">新 增</a>') ?>
						</div>
						<div class="clearBoth"></div>
					</div>
					<div class="clearBoth"></div>
			...
			<div class="splist_down1">
				...
				<div class="yuandian_xx" id="operate_row" data-id="0">
					<ul>
						<? chekurl($arr,'<li><a href="javascript:" _href="?&m=system&s=gift&a=add" onclick="edit_anli()"><img src="images/biao_31.png"> 编辑</a></li>') ?>
			
						<? chekurl($arr,'<li><a href="javascript:" _href="?m=system&s=change&a=delProduct" onclick="z_confirm(\'确定要删除该可兑商品吗？\',del_anli,\'\');"><img src="images/biao_32.png"> 删除</a></li>') ?>
					</ul>
				</div>
	...
	<script type="text/javascript">
		...
		layui.use(['laydate', 'laypage','table','form'], function(){
		  ...
		  ,active = {
		  	setTags:function(){
				layer.open({
					...
					'<div class="spxx_shanchu_tanchu_01_left">'+'设置可兑商品标签'+
					'</div>'+
		  ...
		  productListTalbe = table.render({
		    elem: '#product_list'
		    ,height: "full-140"
		    ,url: '?m=system&s=change&a=getProductList&id=<?=$id?>'
		    ,page: {curr:<?=$page?>}
		    ,limit:<?=$limit?>
		    ,cols: [[<?=$rowsJS?>]]
		    ,where:{
		...
		function edit_anli(params){
        	...
        	location.href="?m=system&s=gift&a=add&id="+pdtId+"&url="+url;
        }
        ...
        function del_anli(params){
        	...
        	ajaxpost=$.ajax({
        		type: "POST",
        		url: "?m=system&s=change&a=delProduct",
        		data: "&ids="+pdtId,
        		dataType:"json",timeout : 8000,
        		success: function(resdata){
        			if(resdata.code==1){
