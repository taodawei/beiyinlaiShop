...
$allRows = array(
				"title"=>array("title"=>"Excel名称","rowCode"=>"{field:'title',title:'Excel名称',width:200}"),
                "field_title"=>array("title"=>"数据字段","rowCode"=>"{field:'field_title',title:'数据字段',width:200}"),
                "type"=>array("title"=>"类型","rowCode"=>"{field:'type',title:'类型',width:80}"),
...
<body>
	<div class="right_up">
		<img src="images/biao_19.png"/> 商品参数列表
	</div>
     ...
     <!--               </div>-->
					<div class="splist_up_01_right">	
						<div class="splist_up_01_right_1">
							<div class="splist_up_01_right_1_left">
								<input type="text" id="keyword" value="<?=$keyword?>" placeholder="请输入Excel名称/数据字段"/>
							</div>
						...
						<div class="splist_up_01_right_3">
						    <? chekurl($arr,'<a href="?&m=system&s=adminlist&a=addParam" class="splist_add">新 增</a>') ?>
						</div>
						<div class="clearBoth"></div>
					</div>
					<div class="clearBoth"></div>
			...
			<div class="splist_down1">
				...
				<div class="yuandian_xx" id="operate_row" data-id="0">
					<ul>
						<? chekurl($arr,'<li><a href="javascript:" _href="?&m=system&s=adminlist&a=addParam" onclick="edit_anli()"><img src="images/biao_31.png"> 编辑</a></li>') ?>
						...
						<? chekurl($arr,'<li><a href="javascript:" _href="?&m=system&s=adminlist&a=delParam" onclick="z_confirm(\'确定要删除该参数字段吗？\',del_anli,\'\');"><img src="images/biao_32.png"> 删除</a></li>') ?>
					</ul>
				</div>
	...
	<script type="text/javascript">
		...
		layui.use(['laydate', 'laypage','table','form'], function(){
		  ...
		  productListTalbe = table.render({
		    elem: '#product_list'
		    ,height: "full-140"
		    ,url: '?m=system&s=adminlist&a=getParamList'
		    ,page: {curr:<?=$page?>}
		    ,limit:<?=$limit?>
		    ,cols: [[<?=$rowsJS?>]]
		    ,where:{
		...
		function edit_anli(params){
        	...
        	var url = '?m=system&s=adminlist&a=param&channelId='+channelId+"&brandId="+brandId+"&status="+status+"&keyword="+keyword+"&tags="+tags+"&source="+source+"&cuxiao="+cuxiao+"&page="+page+"&order1="+order1+"&order2="+order2;
        	url = encodeURIComponent(url);
        	location.href="?m=system&s=adminlist&a=addParam&id="+pdtId+"&url="+url;
        }
        ...
        function del_anli(params){
        	...
        	ajaxpost=$.ajax({
        		type: "POST",
        		url: "?&m=system&s=adminlist&a=delParam",
        		data: "&ids="+pdtId,
        		dataType:"json",timeout : 8000,
        		success: function(resdata){
        			if(resdata.code==1){
