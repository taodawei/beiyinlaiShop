...
$occasionArr = [];
if(!empty($id)){
	$channel = $db->get_row("select * from demo_search_channel where comId=$comId and id = {$id}");
	if($channel->tags){
...
<body>
	<div class="jiliangdanwei">
		<div class="jiliangdanwei_up">
			...
			<div class="jiliangdanwei_up_right">
				<a href="?m=system&s=adminlist&a=search">列表</a>
			</div>
			<div class="clearBoth"></div>
		</div>
		<div class="shangpinguanli" style="padding-top:20px">
            <form action="?m=system&s=adminlist&a=addProductChannel&submit=1&id=<?=$id?>" class="layui-form" method="post">
                    ...
                    <div class="layui-form-item">
                     ...
                     <div class="layui-form-item">
                        <label  class="layui-form-label"><span>*</span>字段名称</label>
                        <div class="layui-input-block" style="width: 500px;">
                            <input type="text" class="layui-input" placeholder="分类名称" name="field_title" value="<?=$channel?$channel->field_title:''?>">
                        </div>
                    </div>
                    ...
                    <div class="layui-form-item" style="display:none;">
                        <div class="layui-inline">
                            <label  class="layui-form-label"><span>*</span>所属属性</label>
                            <div class="layui-input-block">
                                <input type="radio" name="type" value="0" title="视频" <? if(empty($channel->type)){?>checked="checked"<? }?>>
                                <input type="radio" name="type" value="1" title="图文" <? if($channel->type == 1){?>checked="checked"<? }?>>
                                <input type="radio" name="type" value="2" title="文件下载" <? if($channel->type == 2){?>checked="checked"<? }?>>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="layui-form-item" style="display:none;">
                        <label  class="layui-form-label"><span>*</span>推荐图片</label>
                        <div class="layui-input-block" style="width: 120px;position:relative">
                    ...
                    <div class="layui-form-item" style="margin-top: 30px;">
                        ...
                        <div class="layui-input-block">
                            ...
                            <a href="?m=system&s=adminlist&a=search" class="layui-btn layui-btn-primary">返回列表</a>
                        </div>
                    </div>
	...
	<script>
        layui.use(['form','upload'],function(){
            ...
            $.get("/erp_service.php?action=get_search_channels&id=<?=$id?>&pid=<?=$pid?>",function(res){
