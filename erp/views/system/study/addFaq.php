...
if($id > 0){
    $banner = $db->get_row("select * from demo_faq where id = $id and is_del = 0 ");
    $details = $db->get_results("select * from demo_faq_detail where faqId = $id ");
}
...
<body>
	<form action="?m=system&s=study&a=addFaq&submit=1&type=<?=$type?>" method="post" id="createPdtForm" class="layui-form" enctype="multipart/form-data">
		<input type="hidden" name="url" value="<?=$url?>">
		<input type="hidden" name="id" value="<?=$id?>">
		<div class="content_edit">
			<div class="edit_h">
				<a href="?m=system&s=study&a=faq"><img src="images/back.jpg" /></a>
				<span>添加技术问题</span>
			</div>
			<br>
                ...
                <table class="layui-table" id="serviceTables">
                    <colgroup>
                        ...
                        <tr>
                            ...
                            <th><span>回复</span><span class="layui-table-sort layui-inline" style="margin-top: 5px"></th>
                            <th>操作</th>
                        </tr>
							...
							foreach ($details as $detail){
								...
								<tr>
                                    <td>
                                        ...
                                        <textarea name="addRowKey[]" style="width:500px;height:200px;border: medium dashed green;" class="layui-textarea" ><?=$detail->question?></textarea>
                                    </td>
                                   <td>
                                       <!--<input type="text" name="addRowValue[]"  lay-verify="required"  value="<?=$custom?>" autocomplete="off" class="layui-input">-->
                                       <textarea name="addRowValue[]" style="width:500px;height:200px;border: medium dashed green;" class="layui-textarea" ><?=$detail->answer?></textarea>
                                   </td>
                                    <td>
    ...
    layui.use(['upload','form','layer'], function(){
        ...
        $("#newBtn").click(function() {
            ...
            str += '<tr>';
            str += '<td>';
            // str += '<input type="text" style="height:40px;" name="addRowKey[]" lay-verify="required" placeholder="请输入附件名称" autocomplete="off"';
            // str += 'class="layui-input">';
            
            str += '<textarea name="addRowKey[]" style="width:500px;height:200px;border: medium dashed green;" class="layui-textarea" placeholder="输入问题"></textarea>';
  
            str += '</td>';
            // str += '<td><input type="text" style="height:40px;" name="addRowValue[]"  lay-verify="required"  placeholder="请输入附件下载链接" autocomplete="off"';
            // str += 'class="layui-input"></td>';   
             
            str += '<td><textarea name="addRowValue[]" style="width:500px;height:200px;border: medium dashed green;" class="layui-textarea" placeholder="输入回复"></textarea></td>';
