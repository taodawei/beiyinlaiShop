...
$allRows = array(
				"title"=>array("title"=>"名称","rowCode"=>"{field:'title',title:'名称',width:200}"),
                // "channelTitle"=>array("title"=>"分类","rowCode"=>"{field:'channelTitle',title:'分类',width:120}"),
				// "num"=>array("title"=>"招聘人数","rowCode"=>"{field:'num',title:'招聘人数',width:120}"),
				"en_title"=>array("title"=>"英文名称","rowCode"=>"{field:'en_title',title:'英文名称',width:200}"),
				"statusInfo"=>array("title"=>"状态","rowCode"=>"{field:'statusInfo',title:'状态',width:80}"),
				"ordering"=>array("title"=>"排序","rowCode"=>"{field:'ordering',title:'排序',width:80,sort:true}"),
				"dtTime"=>array("title"=>"发布日期","rowCode"=>"{field:'dtTime',title:'发布日期',width:180,sort:true}"),
			);
...
<body>
	<div class="right_up">
		<img src="images/biao_19.png"/> 实验步骤列表
	</div>
     ...
     <!--               </div>-->
					<div class="splist_up_01_right">	
						<div class="splist_up_01_right_1">
							<div class="splist_up_01_right_1_left">
								<input type="text" id="keyword" value="<?=$keyword?>" placeholder="请输入问题名称/问题内容"/>
							</div>
						...
						<div class="splist_up_01_right_3">
						    <? chekurl($arr,'<a href="?&m=system&s=study&a=addStep" class="splist_add">新 增</a>') ?>
						</div>
						<div class="clearBoth"></div>
					</div>
					<div class="clearBoth"></div>
			...
			<div class="splist_down1">
				...
				<div class="yuandian_xx" id="operate_row" data-id="0">
					<ul>
						<? chekurl($arr,'<li><a href="javascript:" _href="?&m=system&s=study&a=addStep" onclick="edit_anli()"><img src="images/biao_31.png"> 编辑</a></li>') ?>
						...
						<? chekurl($arr,'<li><a href="javascript:" _href="?&m=system&s=study&a=delStep" onclick="z_confirm(\'确定要删除该实验步骤吗？\',del_anli,\'\');"><img src="images/biao_32.png"> 删除</a></li>') ?>
					</ul>
				</div>
	...
	<script type="text/javascript">
		...
		layui.use(['laydate', 'laypage','table','form'], function(){
		  ...
		  productListTalbe = table.render({
		    elem: '#product_list'
		    ,height: "full-140"
		    ,url: '?m=system&s=study&a=getStepList'
		    ,page: {curr:<?=$page?>}
		    ,limit:<?=$limit?>
		    ,cols: [[<?=$rowsJS?>]]
		    ,where:{
		...
		function edit_anli(params){
        	...
        	var url = '?m=system&s=study&a=faq&channelId='+channelId+"&brandId="+brandId+"&status="+status+"&keyword="+keyword+"&tags="+tags+"&source="+source+"&cuxiao="+cuxiao+"&page="+page+"&order1="+order1+"&order2="+order2;
        	url = encodeURIComponent(url);
        	location.href="?m=system&s=study&a=addStep&id="+pdtId+"&url="+url;
        }
        ...
        function del_anli(params){
        	...
        	ajaxpost=$.ajax({
        		type: "POST",
        		url: "?&m=system&s=study&a=delStep",
        		data: "&ids="+pdtId,
        		dataType:"json",timeout : 8000,
        		success: function(resdata){
        			if(resdata.code==1){
