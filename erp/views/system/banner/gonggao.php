...
if(strstr($arr, '?m=system&s=banner&a=totop')){
    $allRows = array(
				"title"=>array("title"=>"标题","rowCode"=>"{field:'title',title:'标题',width:300}"),
				"channel"=>array("title"=>"分类","rowCode"=>"{field:'channel',title:'分类',width:180}"),
				...
				"if_show"=>array("title"=>"展示","rowCode"=>"{field:'if_show',title:'展示',width:100}"),
				"totop"=>array("title"=>"置顶","rowCode"=>"{field:'totop',title:'置顶',width:100}"),
				"dtTime"=>array("title"=>"发布时间","rowCode"=>"{field:'dtTime',title:'发布时间',width:200,sort:true}")
			);
}else{
     $allRows = array(
				"title"=>array("title"=>"标题","rowCode"=>"{field:'title',title:'标题',width:300}"),
				"channel"=>array("title"=>"分类","rowCode"=>"{field:'channel',title:'分类',width:300}"),
				
				"language"=>array("title"=>"语言","rowCode"=>"{field:'language',title:'语言',width:150}"),
				
				"url"=>array("title"=>"链接地址","rowCode"=>"{field:'url',title:'链接地址',width:300}"),
				"ordering"=>array("title"=>"权重","rowCode"=>"{field:'ordering',title:'权重',width:100}"),
				// "if_index"=>array("title"=>"推荐","rowCode"=>"{field:'if_index',title:'推荐',width:100}"),
				// "totop"=>array("title"=>"置顶","rowCode"=>"{field:'totop',title:'置顶',width:100}"),
				"dtTime"=>array("title"=>"发布时间","rowCode"=>"{field:'dtTime',title:'发布时间',width:200,sort:true}")
			);
}
...
$channels = $db->get_results("select * from demo_list_channel where comId=$comId and parentId = 0");
foreach ($channels as &$channel){
    $channel->channels = $db->get_results("select * from demo_list_channel where comId=$comId and parentId = $channel->id");
}
...
<body>
	<div class="right_up">
		<img src="images/biao_131.png"/> 资讯列表
	</div>
		...
		<div class="splist">
				...
				<div class="splist_up_01">
					...
					<div class="splist_up_01_right">
						...
						<div class="splist_up_01_right_3">
						    <? chekurl($arr,'<a href="?m=system&s=banner&a=addGonggao&source='.$source.'" class="splist_add">新 增</a>') ?>
			...
			<div class="splist_down1">
				...
				<div class="yuandian_xx" id="operate_row" data-id="0">
					<ul>
						<li>
							<a href="javascript:view();"><img src="images/biao_136.png"> 修改</a>
						</li>
						<li>
	...
	<script type="text/javascript">
		...
		layui.use(['laydate', 'laypage','table','form','upload'], function(){
		  ...
		  productListTalbe = table.render({
		    elem: '#product_list'
		    ,height: "full-140"
		    ,url: '?m=system&s=banner&a=getGonggaos&source=<?=$source?>'
		    ,page: {curr:<?=$page?>}
		    ,limit:<?=$limit?>
		    ,cols: [[<?=$rowsJS?>]]
		    ,where:{
		...
		function loadZiChannels(menuId,ceng,hasnext){
        	...
        	if(hasnext==1){
        		ajaxpost=$.ajax({
        			type: "POST",
        			url: "/erp_service.php?action=get_wenzhang_channels",
        			data: "&id="+menuId,
