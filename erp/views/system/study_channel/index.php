	...
	<div class="jiliangdanwei">
		<div class="jiliangdanwei_up">
			<div class="jiliangdanwei_up_left">
				<img src="images/biao_35.png"/> 学习中心分类管理
			</div>
			<div class="jiliangdanwei_up_right">
			    <? chekurl($arr,'<a href="javascript:" _href="?m=system&s=study_channel&a=addProductChannel" onclick="edit_channel(0,0,\'\',\'\',\'\',0,\'\');">+ 新 增</a>') ?>
			</div>
				...
				if(!empty($channels)){
					foreach ($channels as $c){
						...
						if(!empty($channels1)){
							...
							$ifhas = $db->get_var("select id from demo_study where is_del=0  and channelId=$c->id limit 1");
						...
						<li data-id="<?=$c->id?>" data-pid="0">
							<div class="shangpinguanli_01">
								...
								<div class="shangpinguanli_01_right">
								    ...
								    <? chekurl($arr,'<a href="javascript:" _href="?m=system&s=study_channel&a=addProductChannel" onclick="edit_channel(0,'.$c->id.',\''.$c->title.'\',\''.$c->en_title.'\',\''.$c->originalPic.'\',\''.$c->backimg.'\',\''.$c->is_hot.'\',\''.$c->type.'\',\''.$c->remark.'\');"><img src="images/biao_49.png"/> 新增子类</a>') ?>
									<? chekurl($arr,'<a href="javascript:" _href="?m=system&s=study_channel&a=addProductChannel" onclick="edit_channel('.$c->id.',0,\''.$c->title.'\',\''.$c->en_title.'\',\''.$c->originalPic.'\',\''.$c->backimg.'\',\''.$c->is_hot.'\',\''.$c->type.'\',\''.$c->remark.'\');"><img src="images/biao_49.png"/> 修改</a>') ?>
									...
									<? chekurl($arr,'<a href="javascript:" _href="?m=system&s=study_channel&a=delChannel" '.($ifdele==1?'onclick="z_confirm(\'确定要删除“'.$c->title.'”分类吗？\',delChannel,'.$c->id.');"':'style="color:#999;opacity:.7"').'><img src="images/biao_48.png"/> 删除</a>') ?>
								</div>
								<div class="clearBoth"></div>
							</div>
							<?
									...
									<ul>
										...
										foreach ($channels1 as $c1) {
											...
											if(!empty($channels2)){
												...
												$ifhas = $db->get_var("select id from demo_study where is_del=0  and channelId=$c1->id limit 1");
												$ifdele = empty($ifhas)?1:0;
											}
											?>
														...
														<ul>
															...
															foreach ($channels2 as $c2) {
																...
																if(!empty($channels3)){
																	...
																	$ifhas = $db->get_var("select id from demo_study where is_del=0  and channelId=$c2->id limit 1");
																	$ifdele = empty($ifhas)?1:0;
																}
																?>
																		...
																		<div class="shangpinguanli_04" pid="<?=$c2->id?>" <? if(in_array($c2->id,$showArry)){?>style="display:block"<? }?>>
																			<ul>
																				...
																				foreach ($channels3 as $c3) {
																					$ifhas = $db->get_var("select id from demo_study where is_del=0  and channelId=$c3->id limit 1");
																					$ifdele = empty($ifhas)?1:0;
																					?>
																					<li data-id="<?=$c3->id?>" data-pid="<?=$c2->id?>">
																						<div class="shangpinguanli_01">
																							...
																							<div class="shangpinguanli_01_right">
																								...
																								<a href="javascript:" onclick="edit_channel(<?=$c3->id?>,<?=$c2->id?>,'<?=$c3->title?>','<?=$c3->originalPic?>','<?=$c3->backimg?>','<?=$c3->is_hot?>','<?=$c3->type?>','<?=$c3->remark?>');"><img src="images/biao_49.png"/> 修改</a><a href="?m=system&s=study_channel&a=move&type=1&id=<?=$c3->id?>"><img src="images/biao_33.png"/> 向上</a><a href="?m=system&s=study_channel&a=move&type=0&id=<?=$c3->id?>"><img src="images/biao_34.png"/>向下</a><a href="?m=system&s=product_channel&a=tohot&id=<?=$c->id?>"><a href="javascript:" <? if($ifdele==1){?>onclick="z_confirm('确定要删除“<?=$c3->title?>”分类吗？',delChannel,<?=$c3->id?>);"<? }else{?>style="color:#999;opacity:.7"<? }?>><img src="images/biao_48.png"/> 删除</a>
																							</div>
																							<div class="clearBoth"></div>
																						</div>
																					</li>
