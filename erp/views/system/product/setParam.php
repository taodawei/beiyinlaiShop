...
$param = $db->get_row("select * from demo_product_params where productId = $inventory->productId");
$list = [];
// echo '<pre>';
if($param){
    $allKeys = array_keys((array)$param);
    foreach ($allKeys as $k){
        if(in_array($k, ['id', 'dtTime', 'channelId', 'brandId', 'productId'])){
            continue;
        }
        $temp = array();
        $temp['title'] = $k;
        $temp['name'] = $k;
        $nowTitle = $db->get_var("select title from demo_product_fields where field_title = '$k' and is_del = 0 ");
        if($nowTitle){
            $temp['name'] = $nowTitle;
        }
        $temp['value'] = $param->$k;
...
<body>
	<form action="?m=system&s=product&a=setParam&submit=1" method="post" id="createPdtForm" class="layui-form" enctype="multipart/form-data">
		...
		<div class="content_edit">
			<div class="edit_h">
				...
				<span><?=$inventory->title?> 参数设置</span>
			</div>
                ...
                <table class="layui-table" id="serviceTables">
                    <colgroup>
                        ...
                        <tr>
                            ...
                            <th><span>参数值</span><span class="layui-table-sort layui-inline" style="margin-top: 5px"></th>
                            <!--<th>操作</th>-->
                        </tr>
							...
							foreach ($list as $ck => $custom){
								...
								<tr>
                                    ...
                                    <td>
                                        ...
                                        <?=$custom['name']?>
                                        <input type="hidden" name="addRowKey[]"  value="<?=$custom['title']?>" >
                                    </td>
                                   <td>
                                       <input type="text" name="addRowValue[]"  value="<?=$custom['value']?>" autocomplete="off" class="layui-input">
                                   </td>
                                    <!--<td>-->
