...
$limit = 10;
//计算各类型订单的数量
$num_sql = "select status,count(*) as num from pdt_order_comment where comId=$comId group by status";
$zongNum = 0;
$numArry = array();
		...
		<div class="splist">
			<div class="splist_up" style="height:118px;">
				<div class="splist_up_addtab">
	            	<ul>
	                    <li>
	                    	<a href="?s=order1&a=comment&status=1" <? if($status==1){?>class="splist_up_addtab_on"<? }?>>待审核(<?=(int)$numArry[1]?>)</a>
	                    </li>
	                    <li>
	                    	<a href="?s=order1&a=comment&status=2" <? if($status==2){?>class="splist_up_addtab_on"<? }?>>已审核(<?=(int)$numArry[2]?>)</a>
	                    </li>
	                    <li>
	                    	<a href="?s=order1&a=comment&status=3" <? if($status==3){?>class="splist_up_addtab_on"<? }?>>已回复(<?=(int)$numArry[3]?>)</a>
	                    </li>
...
<script type="text/javascript" src="js/order1/order_comment.js"></script>
