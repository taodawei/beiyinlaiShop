...
//计算各类型订单的数量
$num_sql = "select status,count(*) as num from order_tuihuan where comId=10 and type=$type group by status";
$zongNum = 0;
$numArry = array();
		...
		<div class="splist">
			<div class="splist_up" style="height:118px;">
				<div class="splist_up_addtab">
	            	<ul>
	                    <li>
	                    	<a href="?s=order&a=tuikuan_order&status=1&type=<?=$type?>" <? if($status==1){?>class="splist_up_addtab_on"<? }?>>待审核(<?=(int)$numArry[1]?>)</a>
	                    </li>
	                    <? if($type!=1){?>
		                    <li>
		                    	<a href="?s=order&a=tuikuan_order&status=2&type=<?=$type?>" <? if($status==2){?>class="splist_up_addtab_on"<? }?>>待收货(<?=(int)$numArry[2]?>)</a>
		                    </li>
	                	<? }
	                	if($type!=3){
	                	?>
	                    <li>
	                    	<a href="?s=order&a=tuikuan_order&status=3&type=<?=$type?>" <? if($status==3){?>class="splist_up_addtab_on"<? }?>>待退款(<?=(int)$numArry[3]?>)</a>
	                    </li>
	                    <? 
	                	}
	                    if($type==3){?>
		                    <li>
		                    	<a href="?s=order&a=tuikuan_order&status=4&type=<?=$type?>" <? if($status==4){?>class="splist_up_addtab_on"<? }?>>换货发货(<?=(int)$numArry[4]?>)</a>
		                    </li>
		                    <li>
		                    	<a href="?s=order&a=tuikuan_order&status=5&type=<?=$type?>" <? if($status==5){?>class="splist_up_addtab_on"<? }?>>待客户收货(<?=(int)$numArry[5]?>)</a>
		                    </li>
	                	<? }?>
	                    <li>
	                    	<a href="?s=order&a=tuikuan_order&status=6&type=<?=$type?>" <? if($status==6){?>class="splist_up_addtab_on"<? }?>>已完成(<?=(int)$numArry[6]?>)</a>
	                    </li>
	                    <li>
	                    	<a href="?s=order&a=tuikuan_order&status=-1&type=<?=$type?>" <? if($status==-1){?>class="splist_up_addtab_on"<? }?>>已驳回(<?=(int)$numArry[-1]?>)</a>
	                    </li>
