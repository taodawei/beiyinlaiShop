...
if(!empty($jilus)){
	...
	foreach ($jilus as $jilu){
		...
		if(!empty($inventory)&&!in_array($jilu[0],$hasSns)){
			...
			if($status==1){
				$db->query("update demo_kucun set kucun=kucun+$num,chengben='$chengben' where inventoryId=$inventory->id and storeId=$storeId limit 1");
				$db->query("update demo_product_inventory set kucun=kucun+$num where id=$inventory->id");
			}else{
				//$db->query("update demo_kucun set zaituNum=zaituNum+$num where inventoryId=$inventory->id and storeId=$storeId limit 1");
			}
			$rukuSql1.=",($comId,$rukuId,".$inventory->id.",".$inventory->productId.",'$pdtInfo',$storeId,'$storeName','$num',$status,'$kucun','".$jilu[7]."',1,'$typeInfo','".date("Y-m-d H:i:s")."','".$jilu[3]."','$rukuChengben','$zongchengben')";
		}else{
			$errorJilus[] = $jilu;
		}
	}
	if(!empty($rukuSql1)){
		...
		if($status==0){
			$db->query("insert into demo_task$fenbiao(comId,type,infoId,title,userIds,content,dtTime) value($comId,$type,$rukuId,'新的入库需要您审批','$shenheUser','有新的入库需要您审批，请及时处理','".date("Y-m-d H:i:s")."')");
				//推送消息
			...
			?>
				layer.confirm('无法获取到导入的数据，请重新导入', {
				  btn: ['确定'],
					...
					location.href='?m=system&s=ruku&a=daoru&storeId=<?=$storeId?>';
				});
			<? }
			?>
		});
...
<body>
	<div class="right_up">
    	<img src="images/biao_77.png"/> 商品入库导入
    </div>
    	...
    	<div class="kucunpandian">
        	...
        	<div class="kucunpandian_daorushibai">
            	...
            	<div class="kucunpandian_daorushibai_03">
            		<form id="pandianForm" action="?m=system&s=ruku&a=daochuExcel" method="post" target="_blank">
            	...
            	<div class="kucunpandian_daorushibai_04">
                	<a href="?m=system&s=ruku&a=daoru&storeId=<?=$storeId?>">重新上传</a>
                </div>
