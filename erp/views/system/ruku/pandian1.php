...
<head>
	...
	<script type="text/javascript">
		layui.use(['layer'], function(){
			...
			<? }else if(count($pandians[0])!=8){
				$confirm = 0;
				?>
				layer.confirm('数据格式有误，请先下载库存文件，修改好“入库数量”后上传', {
					btn: ['确定'],
				}, function(){
					location.href='?m=system&s=ruku&a=daoru&storeId=<?=$storeId?>';
				});
				...
				foreach ($pandians as $data) {
					if(!is_numeric($data[4])||!is_numeric($data[5])||$data[5]<0){
						$confirm = 0;
						?>
						layer.confirm('数据格式有误，请确保每一条数据入库数量都大于0', {
							btn: ['确定'],
						}, function(){
							location.href='?m=system&s=ruku&a=daoru&storeId=<?=$storeId?>';
						});
    	...
    	<div class="kucunpandian">
        	...
        	<div class="kucunpandian_yulan">
            	<div class="kucunpandian_yulan_01">
                	前 <?=$num?> 条数据预览  &nbsp;&nbsp;     入库仓库：<?=$db->get_var("select title from demo_kucun_store where id=$storeId")?>  &nbsp;&nbsp;    入库类型：<?=$request['type_info']?>    &nbsp;&nbsp;    入库时间：<?=date("Y-m-d H:i")?>    &nbsp;&nbsp;   制表人：<?=$_SESSION[TB_PREFIX.'name']?> 
                </div>
        	...
        	<div class="kucunpandian_03">
        		<? if($confirm==1){?>
        		<form id="pandianForm" action="?m=system&s=ruku&a=pandian2&storeId=<?=$storeId?>" method="post">
        			<input type="hidden" name="pandianJsonData" value='<?=$pandianJsonData?>'>
        			<input type="hidden" name="filepath" value="<?=$filepath?>">
        			<input type="hidden" name="type_info" value="<?=$request['type_info']?>">
            	</form>
