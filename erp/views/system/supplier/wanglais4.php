...
<body>
    ...
    <div class="yuandian_xx" id="operate_row" data-id="0">
        <ul>
            <li>
                <a href="javascript:detail('');"><img src="images/jsmx.png"> 结算明细</a>
            </li>
        </ul>
    </div>
    ...
    <script type="text/javascript">
        ...
        layui.use(['laypage','table'], function(){
          ...
          productListTalbe = table.render({
            elem: '#product_list'
            ,height: "full-20"
            ,url: '?m=system&s=supplier&a=getWanglai4&id=<?=$id?>'
            ,page: {curr:<?=$page?>}
            ,cols: [[{field: 'id', title: 'id', width:0, sort: true,style:"display:none;"},{field: 'orderId', title: '结算单号'},{field:'dtTime',title:'结算时间',sort:true},{field:'money',title:'结算金额'},{field:'username',title:'经办人'},{fixed:'right',width:49,title:'',align:'center', toolbar: '#barDemo'}]]
            ,where:{
                startTime:'<?=$startTime?>',
                endTime:'<?=$endTime?>'
        ...
        function detail(action){
            ...
            var endTime = $("#endTime").val();
            var url = $("#url").val();
            var returnurl = "?m=system&s=supplier&a=wanglais&id=<?=$id?>&nowPage=4&startTime="+startTime+"&endTime="+endTime;
            returnurl = encodeURIComponent(returnurl);
            parent.location.href="?m=system&s=supplier&a=jiesuan_detail&supplierId=<?=$id?>&id="+jiluId+"&returnurl="+returnurl+"&url="+url;
        }
        //获取当前选中的产品Id
        function getPdtId(){
