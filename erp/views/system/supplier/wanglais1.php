    ...
    <script type="text/javascript">
        ...
        layui.use(['laypage','table'], function(){
          ...
          productListTalbe = table.render({
            elem: '#product_list'
            ,height: "full-20"
            ,url: '?m=system&s=supplier&a=getWanglai1&id=<?=$id?>'
            ,page: {curr:<?=$page?>}
            ,cols: [[{field: 'id', title: 'id', width:0, sort: true,style:"display:none;"},{field: 'orderId', title: '采购单号'},{field:'dtTime',title:'采购时间',sort:true},{field:'price',title:'采购金额'},{field:'price_type',title:'采购方式'},{field:'price_payed',title:'预付款'},{field:'price_weikuan',title:'未付款'},{field:'username',title:'经办人'},{fixed:'right',width:49,title:'',align:'center', toolbar: '#barDemo'}]]
            ,where:{
                startTime:'<?=$startTime?>',
                endTime:'<?=$endTime?>'
        ...
        function detail(action){
            ...
            var endTime = $("#endTime").val();
            var url = $("#url").val();
            var returnurl = "?m=system&s=supplier&a=wanglais&id=<?=$id?>&nowPage=1&startTime="+startTime+"&endTime="+endTime;
