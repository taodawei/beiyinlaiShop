...
header("Content-Disposition:attachment; filename=".$supplier->title."—往来账-赊购采购单.xls");
$allRows = array(
	"orderId"=>"采购单号",
	"dtTime"=>"采购时间",
	"price"=>"采购金额",
...
$startTime = $request['startTime'];
$endTime = $request['endTime'];
$sql="select id,orderId,dtTime,price_type,price,price_weikuan,price_payed,username from demo_caigou where comId=$comId and supplierId=$supplierId and status=1 and price_type=2";
	if(!empty($startTime)){
