...
header("Content-Disposition:attachment; filename=".$supplier->title."—采购单记录-".date("Y-m-d").".xls");
$allRows = array(
				...
				"num"=>array("title"=>"采购数量","rowCode"=>"{field:'num',title:'采购数量',width:250}"),
				"username"=>array("title"=>"制单人","rowCode"=>"{field:'username',title:'制单人',width:100}"),
				"price_type"=>array("title"=>"采购方式","rowCode"=>"{field:'price_type',title:'采购方式',width:100,sort:true}"),
				"price"=>array("title"=>"采购金额","rowCode"=>"{field:'price',title:'采购金额',width:100,sort:true}"),
				"price_weikuan"=>array("title"=>"欠款金额","rowCode"=>"{field:'price_weikuan',title:'欠款金额',width:150}"),
...
<table border="1">   
	...
	if(!empty($pdts)){
		foreach ($pdts as $pdt){
			...
			$pdt->price_type = $price_type;
			$pdt->dtTime = date("Y-m-d H:i",strtotime($pdt->dtTime));
			//$pdt->price = getXiaoshu($pdt->price,$product_set->price_num);
			//$pdt->price_payed = getXiaoshu(($pdt->price-$pdt->price_payed),$product_set->price_num);
			$pdt->num = $db->get_var("select sum(num) from demo_caigou_detail$fenbiao where jiluId=".$pdt->id);
