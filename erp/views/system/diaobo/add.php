...
$orderId = $kucun_set->diaobo_pre.'_'.date("Ymd").'_'.getOrderId($comId,3);
$channels = array();
...
<body>
	<div class="right_up">
    	<a href="javascript:history.go(-1);"><img src="images/biao_63.png"/> 新增商品调拨</a>
    </div>
	<div class="right_down">
    	<div class="sprukuadd">
    		<form id="addForm" action="?m=system&s=diaobo&a=add&tijiao=1" method="post" class="layui-form">
        	<div class="sprukuadd_01">
                ...
                <div class="sprukuadd_01_canku">
                    ...
                    <div class="sprukuadd_01_canku_right">
                        <select name="store1Id" id="store1Id">
            	...
            	<ul>
            		<li>
                        ...
                        <div class="sprukuadd_04_right">
                        	<a href="index.php?a=shezhi&url=%3Fm%3Dsystem%26s%3Dkucun_set%26a%3Dchuruku" target="_blank">设置调拨单号 ></a>
                        </div>
                    	<div class="clearBoth"></div>
                    </li>
                    <li>
		...
		layui.use(['laydate', 'laypage','table','form'], function(){
		  		...
		  		if(data.length>0){
			    	...
			    	for (var i = 0; i < data.length; i++) {
						...
						'<td bgcolor="#ffffff" width="175" class="sprukuadd_03_tt" align="center" valign="middle">'+
							'<input type="text" lay-verify="kucun" name="inventoryNum['+num+']" max="'+max+'" onfocus="showTips(this,\''+max+'\');" value="'+shuliang+'" class="sprukuadd_03_tt_input">'+
		  ...
		  form.verify({
            kucun:function(value,item){
                ...
                if(value>max){
                    $(item).focus();
                    return '调拨数量'+value+'不能大于现有库存量'+max;
                }
                <? }?>
            }
          });
          ...
          form.on('submit(tijiao)', function(data){
            if($("#store1Id option:selected").val()==''){
                layer.msg("请选择调入仓库！",function(){});
                return false;
            }
            if($("#storeId option:selected").val()==$("#store1Id option:selected").val()){
                layer.msg("调出仓库与调入仓库不能相同！",function(){});
                return false;
            }
			layer.load();
		  });
	...
	<script type="text/javascript" src="js/churuku_add.js"></script>
