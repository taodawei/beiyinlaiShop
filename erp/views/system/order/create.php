...
$id = (int)$request['id'];
if(!empty($id)){
    $list = $db->get_row("select * from demo_product_limit where id=$id and comId=$comId");
    $startTime = date('Y-m-d', strtotime($list->startTime));
    $endTime = date('Y-m-d', strtotime($list->endTime));
}
?>
...
<body>
<div class="right_up">
    <a href="?s=product&a=jifenticket"><img src="images/back.gif"/></a> 新增线下卡
</div>
<div class="right_down">
    <div class="sprukuadd">
        <form id="addForm" action="" method="post" class="layui-form">
            <input type="hidden" name="startTime" id="startTime" value="<?=$list->startTime?>">
            <input type="hidden" name="endTime" id="endTime" value="<?=$list->endTime?>">
                ...
                <ul>
                    ...
                    <li>
                        ...
                        <div class="yx_guanggaoadd_01_right">
                            <input type="number" name="number" value="<?=$list->number?>" lay-verify="required" placeholder="请输入发放数量" class="yx_guanggaoadd_01_right_input" min="1"/>
                        </div>
                        <div class="clearBoth"></div>
                    </li>
                    <li>
