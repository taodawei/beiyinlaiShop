...
$allRows = array(
    ...
    "code"=>array("title"=>"卡号","rowCode"=>"{field:'code',title:'卡号',width:220}"),
    "pass"=>array("title"=>"密码","rowCode"=>"{field:'pass',title:'密码',width:200}"),
    "title"=>array("title"=>"产品标题","rowCode"=>"{field:'title',title:'产品标题',width:200}"),
    ...
    "status_info"=>array("title"=>"状态","rowCode"=>"{field:'status_info',title:'状态',width:134}"),
    "status_export"=>array("title"=>"导出状态","rowCode"=>"{field:'status_export',title:'导出状态',width:134}"),
    "startTime"=>array("title"=>"开始时间","rowCode"=>"{field:'startTime',title:'开始时间',width:220}"),
    "endTime"=>array("title"=>"结束时间","rowCode"=>"{field:'endTime',title:'结束时间',width:220}"),
    "dtTime"=>array("title"=>"生成时间","rowCode"=>"{field:'dtTime',title:'生成时间',width:220}"),
        ...
        <div class="splist">
            <div class="splist_up" style="height:118px;">
                <div class="splist_up_addtab">
                    <ul>
                        <li>
                            <a href="?s=order&a=xxk&scene=<?=$scene?>" <? if(empty($type)){?>class="splist_up_addtab_on"<? }?>>全部</a>
                        </li>
                        <li>
                            <a href="?s=order&status=1&a=xxk&type=1&scene=<?=$scene?>" <? if($type==1){?>class="splist_up_addtab_on"<? }?>>未使用</a>
                        </li>
                        <li>
                            <a href="?s=order&status=2&a=xxk&type=2&scene=<?=$scene?>" <? if($type==2){?>class="splist_up_addtab_on"<? }?>>已使用</a>
                        </li>
                    ...
                    <div class="splist_up_01_right">
                        ...
                        <div class="splist_up_01_right_3">
                            <? if($adminRole>=7||strstr($qx_arry['product']['functions'],'daochu')){?>
                               <a href="?m=system&s=order&a=xxkExport" ;" class="splist_daochu">导 出</a>
                            <? }
                            if($adminRole>=7||strstr($qx_arry['product']['functions'],'daoru')){
                                ?>
                                <!--							<a href="?m=system&s=product&a=daoru" class="splist_daoru">导 入</a>-->
                            <? }
                            if($adminRole>=7||strstr($qx_arry['product']['functions'],'edit')){
                                ?>
                                <a href="?m=system&s=order&a=create" class="splist_add">新 增</a>
                            <? }?>
                        </div>
                        <div class="clearBoth"></div>
                    </div>
                ...
                <div class="splist_up_02">
                    ...
                    <? if($type>0){?>
                        <div class="dangqiandd_2_down_3">
                            <a href="?s=fahuo&a=daochu" onclick="daochu();" id="daochu" target="_blank"><img src="images/dangqiandingdan_12.png"> 批量导出</a>
                            <a href="javascript:;" id="zanting_fahuo"><img src="images/order/dangqiandingdan_14.png" /> 暂停发货</a>
                            <? if($comId==1121||$comId==1041){?>
                                <a href="javascript:;" onclick="$('.putongfh_fahuo_tc2').show();"><img src="images/order/dangqiandingdan_14.png" /> 修改发货日期</a>
                                <a href="javascript:;"  onclick="$('.putongfh_fahuo_tc1').show();"><img src="images/order/dangqiandingdan_14.png" /> 批量发货</a>
                            <? }?>
                        </div>
                    <? }?>
                    <div class="clearBoth"></div>
...
<script type="text/javascript">
    ...
    layui.use(['laydate', 'laypage','table','form'], function(){
        ...
        productListTalbe = table.render({
            elem: '#product_list'
            ,height: "full-200"
            ,url: '?s=order&a=xxkList&print_type=1'
            ,page: {curr:<?=$page?>}
            ,limit:<?=$limit?>
            ,cols: [[<?=$rowsJS?>]]
            ,where:{
