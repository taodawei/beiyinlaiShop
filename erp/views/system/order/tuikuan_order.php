...
$allRows = array(
	...
	"order_orderId"=>array("title"=>"订单号","rowCode"=>"{field:'order_orderId',title:'订单号',width:240}"),
	"pdtInfo"=>array("title"=>"商品信息","rowCode"=>"{field:'pdtInfo',title:'商品信息',width:300}"),
	"reason"=>array("title"=>"退换货原因","rowCode"=>"{field:'reason',title:'退换货原因',width:200}"),
	"money"=>array("title"=>"退款总额","rowCode"=>"{field:'money',title:'退款总额',width:100,sort:true}"),
	"username"=>array("title"=>"申请人","rowCode"=>"{field:'username',title:'申请人',width:180}"),
...
//计算各类型订单的数量
$num_sql = "select status,count(*) as num from order_tuihuan where comId=$comId and type=$type and if_jifen = $ifJifen group by status";
$zongNum = 0;
$numArry = array();
...
<div id="content">
		...
		<div class="splist">
			<div class="splist_up" style="height:118px;">
				<div class="splist_up_addtab">
	            	<ul>
	                    <li>
	                    	<a href="?s=order&a=tuikuan_order&status=1&type=<?=$type?>&if_jifen=<?=$ifJifen?>" <? if($status==1){?>class="splist_up_addtab_on"<? }?>>待审核(<?=(int)$numArry[1]?>)</a>
	                    </li>
	                    <? if($type!=1){?>
		                    <li>
		                    	<a href="?s=order&a=tuikuan_order&status=2&type=<?=$type?>&if_jifen=<?=$ifJifen?>" <? if($status==2){?>class="splist_up_addtab_on"<? }?>>待收货(<?=(int)$numArry[2]?>)</a>
		                    </li>
	                	<? }
	                	if($type!=3){
	                	?>
	                    <li>
	                    	<a href="?s=order&a=tuikuan_order&status=3&type=<?=$type?>&if_jifen=<?=$ifJifen?>" <? if($status==3){?>class="splist_up_addtab_on"<? }?>>待退款(<?=(int)$numArry[3]?>)</a>
	                    </li>
	                    <? 
	                	}
	                    if($type==3){?>
		                    <li>
		                    	<a href="?s=order&a=tuikuan_order&status=4&type=<?=$type?>&if_jifen=<?=$ifJifen?>" <? if($status==4){?>class="splist_up_addtab_on"<? }?>>换货发货(<?=(int)$numArry[4]?>)</a>
		                    </li>
		                    <li>
		                    	<a href="?s=order&a=tuikuan_order&status=5&type=<?=$type?>&if_jifen=<?=$ifJifen?>" <? if($status==5){?>class="splist_up_addtab_on"<? }?>>待客户收货(<?=(int)$numArry[5]?>)</a>
		                    </li>
	                	<? }?>
	                    <li>
	                    	<a href="?s=order&a=tuikuan_order&status=6&type=<?=$type?>&if_jifen=<?=$ifJifen?>" <? if($status==6){?>class="splist_up_addtab_on"<? }?>>已完成(<?=(int)$numArry[6]?>)</a>
	                    </li>
	                    <li>
	                    	<a href="?s=order&a=tuikuan_order&status=-1&type=<?=$type?>&if_jifen=<?=$ifJifen?>" <? if($status==-1){?>class="splist_up_addtab_on"<? }?>>已驳回(<?=(int)$numArry[-1]?>)</a>
	                    </li>
	...
	<div class="dqddxiangqing" id="dqddxiangqing" data-id="0" data-tuihuanId="0" style="display:none;">
...
<script type="text/javascript">
	...
	layui.use(['laydate', 'laypage','table','form'], function(){
	  ...
	  productListTalbe = table.render({
	    elem: '#product_list'
	    ,height: "full-200"
	    ,url: '?s=order&a=getTuihuanList&type='+type
	    ,page: {curr:<?=$page?>}
	    ,limit:<?=$limit?>
	    ,cols: [[<?=$rowsJS?>]]
	    ,where:{
