...
$allRows = array(
	...
	"sn"=>array("title"=>"服务编号","rowCode"=>"{field:'sn',title:'服务编号',width:200}"),
	"order_orderId"=>array("title"=>"订单号","rowCode"=>"{field:'orderId',title:'订单号',width:240}"),
	"title"=>array("title"=>"服务项目","rowCode"=>"{field:'title',title:'服务项目',width:300}"),
	"price"=>array("title"=>"服务金额","rowCode"=>"{field:'price',title:'服务金额',width:100}"),
	"ispay"=>array("title"=>"支付状态","rowCode"=>"{field:'ispay',title:'支付状态',width:100}"),
	"dtTime"=>array("title"=>"申请时间","rowCode"=>"{field:'dtTime',title:'申请时间',width:150}"),
	"service_time"=>array("title"=>"预约时间","rowCode"=>"{field:'service_time',title:'预约时间',width:150,sort:true}"),
	"status_info"=>array("title"=>"服务状态","rowCode"=>"{field:'status_info',title:'服务状态',width:120}")
);
if($status>1){
	$allRows["worker_name"] = array("title"=>"服务人员","rowCode"=>"{field:'worker_name',title:'服务人员',width:120}");
	if($status==3){
		$allRows["dealTime"] = array("title"=>"完成时间","rowCode"=>"{field:'dealTime',title:'完成时间',width:150}");
	}
}elseif($status==-1){
	$allRows["worker_name"] = array("title"=>"操作人员","rowCode"=>"{field:'worker_name',title:'操作人员',width:120}");
	$allRows["dealTime"] = array("title"=>"操作时间","rowCode"=>"{field:'dealTime',title:'操作时间',width:150}");
}
$rowsJS = "{type:'checkbox', fixed: 'left'},{field: 'id', title: 'id', width:0,style:\"display:none;\"},{field: 'status', title: 'status', width:0,style:\"display:none;\"},{field: 'serviceId', title: 'serviceId', width:0,style:\"display:none;\"}";
...
//计算各类型订单的数量
$num_sql = "select status,count(*) as num from order_service where comId=$comId group by status";
$zongNum = 0;
$numArry = array();
...
$service_items = get_service_items();
?>
...
<div id="content">
	<div class="right_up">
		<img src="images/biao_109.png"/> 订单服务<?='('.$zongNum.')'?>
		...
		<div class="splist">
			<div class="splist_up" style="height:118px;">
				<div class="splist_up_addtab">
	            	<ul>
	                    <li>
	                    	<a href="?s=order&a=service&status=1" <? if($status==1){?>class="splist_up_addtab_on"<? }?>>待分配(<?=(int)$numArry[1]?>)</a>
	                    </li>
	                    <li>
	                    	<a href="?s=order&a=service&status=2" <? if($status==2){?>class="splist_up_addtab_on"<? }?>>待服务(<?=(int)$numArry[2]?>)</a>
	                    </li>
	                    <li>
	                    	<a href="?s=order&a=service&status=3" <? if($status==3){?>class="splist_up_addtab_on"<? }?>>已完成(<?=(int)$numArry[3]?>)</a>
	                    </li>
	                    <li>
	                    	<a href="?s=order&a=service&status=-1" <? if($status==-1){?>class="splist_up_addtab_on"<? }?>>已取消(<?=(int)$numArry[-1]?>)</a>
	                    </li>
					...
					<div class="splist_up_01_left">
						<div class="splist_up_01_left_02">
							<div class="splist_up_01_left_02_up">
								<span>服务项目</span> <img src="images/biao_20.png"/>
							</div>
							<div class="splist_up_01_left_02_down">
								<ul>
									<li>
										<a href="javascript:" onclick="selectItem('全部');">全部</a>
									</li>
									<? foreach($service_items as $key=>$re){?>
									<li>
										<a href="javascript:" onclick="selectItem(<?=$key?>,'<?=$re?>');"><?=$re?></a>
									</li>
									<? }?>
								</ul>
							</div>
					...
					<div class="splist_up_01_right">
						<div class="splist_up_01_right_1">
							<div class="splist_up_01_right_1_left">
								<input type="text" id="keyword" value="<?=$keyword?>" placeholder="请输入订单号/服务单号/申请人"/>
							</div>
						...
						<div class="dangqiandd_2_up_right_adddingdanfuwu"><a href="javascript:" onclick="service_edit(0,0,1);">新增订单服务</a></div>
						<div class="clearBoth"></div>
					</div>
					<div class="clearBoth"></div>
				</div>
				<div class="splist_up_02">
					...
					<div class="dangqiandd_2_down_3">
						<? if($status==1){?>
							<a href="javascript:" onclick="pi_fenpei();"><img src="images/dingdanfuwu_1.png"> 批量分配</a>
							<a href="javascript:" onclick="pi_zuofei();"><img src="images/dingdanfuwu_11.png"> 批量作废</a>
						<? }else if($status==2){?>
							<a href="javascript:" onclick="pi_wancheng();"><img src="images/dangqiandingdan_1.png"> 批量设为已完成</a>
							<a href="javascript:" onclick="pi_zuofei();"><img src="images/dingdanfuwu_11.png"> 批量作废</a>
						<? }?>
					</div>
					<div class="clearBoth"></div>
			...
			<div class="splist_down1">
				...
				<div class="yuandian_xx" id="operate_row" data-id="0" style="width:100px;">
					<ul>
						<li id="sheheBtn">
							<a href="javascript:" onclick="service_shenhe();"><img src="images/shangchengdd_22.png">服务分配</a>
						</li>
						<li id="doneBtn">
							<a href="javascript:" onclick="service_done();"><img src="images/shangchengdd_22.png">服务完成</a>
						</li>
						<li>
							<a href="javascript:" onclick="service_zuofei();"><img src="images/shangchengdd_25.png">服务作废</a>
						</li>
					</ul>
				</div>
			</div>
	  ...
	  table.on('checkbox(product_list)', function(obj){
	    ...
	    if(data.length>0){
	    	...
	    	for (var i = 0; i < data.length; i++) {
	    		if(i==0){
	    			ids = data[i].serviceId;
	    		}else{
	    			ids = ids+','+data[i].serviceId;
	    		}
	    	}
	    	$("#selectedIds").val(ids);
	    	$(".splist_up_01").hide();
...
<script type="text/javascript" src="js/order/order_service.js"></script>
