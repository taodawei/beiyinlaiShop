...
$id = (int)$request['id'];
$cuxiao = $db->get_row("select * from cuxiao_pdt where id=$id and comId=$comId");
if(empty($cuxiao)){
    die("促销活动不存在");
}
...
<body>
    <div class="right_up">
        <a href="<?=urldecode($request['returnurl'])?>"><img src="images/back.gif"/></a> 商品促销详情
    </div>
    <div class="right_down">
        <div class="yx_shangpincuxiaoxiangqing">
            <div class="yx_shangpincuxiaoxiangqing_01">
                ...
                <!--    <a href="?s=yyyx&a=create_cuxiao&id=<?=$id?>"><img src="images/yingxiao_26.png"/>复制</a><? if($cuxiao->status==1){?><a href="javascript:" onclick="z_confirm('确定要作废该促销吗？',zuofei,<?=$id?>);"><img src="images/yingxiao_27.png"/>作废</a><? }?>-->
                <!--</div>-->
            ...
            <div class="yx_shangpincuxiaoxiangqing_02">
                <ul>
                    ...
                    <li>
                        ...
                        <div class="yx_shangpincuxiaoxiangqing_02_right">
                            <?=date("Y-m-d H:i:s",strtotime($cuxiao->startTime))?> ~ <?=date("Y-m-d H:i:s",strtotime($cuxiao->endTime))?>
                        </div>
                        <div class="clearBoth"></div>
                    </li>
                    <li>
                        ...
                        <div class="yx_shangpincuxiaoxiangqing_02_right">
                            ...
                            foreach ($pdtIds as $inventoryId) {
                                $inventory = $db->get_row("select sn,title,key_vals from demo_product_inventory where id=$inventoryId");
                                $xiangou =  $db->get_row("select num,price from cuxiao_pdt_xiangou where inventoryId=$inventoryId and cuxiao_id = $id");
                                if(!empty($inventory)){
                                    ?>
                                    <a><?=$inventory->title?> <? if($inventory->key_vals!='无'&&$inventory->key_vals!=""){?>【<?=$inventory->key_vals?>】<? }?></a>    <? if($cuxiao->accordType == 3){?>  <span style="color:red">促销价:<?=$xiangou->price?></span>  <span style="color:green;">限购数量：<?=$xiangou->num?></span> <? } ?><br>
                                    <?
                                }
                            }
                            ?>
