...
$id = (int)$request['id'];
if(!empty($id))$cuxiao = $db->get_row("select * from chongzhi_gift where id=$id");
...
<body>
    <div class="right_up">
        <a href="?s=yyyx&a=chongzhi"><img src="images/back.gif"/></a> 新增充值赠送活动
    </div>
                ...
                <li>
                    ...
                    <div class="dhd_adddinghuodan_3_right">
                        <div class="yx_spcuxiaoadd_2_right_guize">
                            <input type="radio" name="type" value="1" lay-filter="type1" <? if(empty($cuxiao)||$cuxiao->type==1){?>checked="true"<? }?> title="充值返现"/><br>
                            <div class="yx_spcuxiaoadd_2_right_guize_01" id="qujian1" rows="<?=$cuxiao->type==1?count($guizes):1?>" <? if(!empty($cuxiao)&&$cuxiao->type!=1){?>style="display:none"<? }?>>
                                <? if($cuxiao->type==1){
                                    foreach ($guizes as $key => $g){
                                        ...
                                        <div id="rows_2_<?=$key+1?>">
                                            充值金额每满<span><input type="number" step="1" name="man_1_<?=$key+1?>" value="<?=$g['man']?>"/></span>，返金额<span><input type="number" step="1" name="jian_1_<?=$key+1?>" value="<?=$g['jian']?>"/></span>元
                                ...
                                <div id="rows_2_1">
                                    充值金额每满<span><input type="number" step="1" name="man_1_1"/></span>，返金额<span><input type="number" step="1" name="jian_1_1"/></span>元
                                    <input type="hidden" name="rows_1[]" value="1" >
                                </div>
                                <? }?>
                                <a href="javascript:" onclick="add_qujian(this,1);">+添加区间</a>
                            </div>
                            <!--<input type="radio" name="type" value="2" lay-filter="type2" <? if($cuxiao->type==2){?>checked="true"<? }?> title="充值返积分" /><br>-->
                            <div class="yx_spcuxiaoadd_2_right_guize_01" id="qujian2" rows="<?=$cuxiao->type==2?count($guizes):1?>" <? if($cuxiao->type!=2){?>style="display:none"<? }?>>
                                <? if($cuxiao->type==2){
                                    foreach ($guizes as $key => $g){
                                        ?>
                                        <div id="rows_2_<?=$key+1?>">
                                            充值金额每满<span><input type="number" step="1" name="man_2_<?=$key+1?>" value="<?=$g['man']?>"/></span>，返积分<span><input type="number" step="1" name="jian_2_<?=$key+1?>" value="<?=$g['jian']?>"/></span>
                                ...
                                <div id="rows_2_1">
                                    充值金额每满<span><input type="number" step="1" name="man_2_1"/></span>，返积分<span><input type="number" step="1" name="jian_2_1"/></span>
                                    <input type="hidden" name="rows_2[]" value="1" >
                                </div>
                                <? }?>
                                <a href="javascript:" onclick="add_qujian(this,2);">+添加区间</a>
                            </div>  
                            <!--<input type="radio" name="type" value="3" lay-filter="type3" <? if($cuxiao->type==3){?>checked="true"<? }?> title="充值赠优惠券" />-->
                            <div class="yx_spcuxiaoadd_2_right_guize_01" id="qujian3" rows="<?=$cuxiao->type==3?count($guizes):1?>" <? if($cuxiao->type!=3){?>style="display:none"<? }?>>
                                <? if($cuxiao->type==3){
                                    foreach ($guizes as $key => $g){
                                        ...
                                        <div id="rows_3_<?=$key+1?>">
                                            订购金额每满<span><input type="number" step="1" name="man_3_<?=$key+1?>" value="<?=$g['man']?>" /></span>，赠优惠券<a href="javascript:" onclick="showAllpdts(<?=$key+1?>);" id="pdt_3_<?=$key+1?>" class="yx_spcuxiaoadd_2_right_guize_01_xuanzesp"><?=$yhqName?></a><span><input type="number" step="1" name="jian_3_<?=$key+1?>" value="<?=$g['jian']?>"/></span>个
                                ...
                                <div id="rows_1_1">
                                    订购金额每满<span><input type="number" step="1" name="man_3_1" /></span>，赠优惠券<a href="javascript:" onclick="showAllpdts(1);" id="pdt_3_1" class="yx_spcuxiaoadd_2_right_guize_01_xuanzesp">请选择优惠券</a><span><input type="number" step="1" name="jian_3_1"/></span>个
...
<script type="text/javascript">
    ...
    form.on('submit(tijiao)', function(data){
        ...
        $.ajax({
            type: "POST",
            url: "?s=yyyx&a=create_chongzhi&submit=1",
            ...
            success: function(data) {
                ...
                if(data.code!=1){
                    ...
                    layer.alert('创建成功',function(){
                        location.href='?s=yyyx&a=chongzhi';
                    });
                }
            },
...
<script type="text/javascript" src="js/yyyx/create_chongzhi.js"></script>
