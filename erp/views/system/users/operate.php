...
$allRows = array(
    "content"=>array("title"=>"操作内容","rowCode"=>"{field:'content',title:'操作内容',width:200}"),
    "dtTime"=>array("title"=>"操作时间","rowCode"=>"{field:'dtTime',title:'操作时间',width:150}"),
    "terminal"=>array("title"=>"操作终端","rowCode"=>"{field:'terminal',title:'操作终端',width:100}"),
    "ip"=>array("title"=>"IP地址","rowCode"=>"{field:'ip',title:'IP地址',width:200}")
);
...
$money = $db->get_var("select sum(price_payed) from order$fenbiao where comId=$comId and userId=$id and status>-1");
if(empty($money))$money=0;
$keyword = $request['keyword'];
if($request['type']==1)$keyword='登录';
                            ...
                            <div class="hyxx_jifenmingxi_up_left">
                                <div class="hyxx_jifenmingxi_up_left_01">
                                    ...
                                    <div class="hyxx_jifenmingxi_up_left_01_right">
                                        <input type="text" id="keyword" value="<?=$keyword?>" placeholder="关键词：“登录”、“修改”等" class="hyxx_xiaofeimingxi_up_left_01_right"/>
                                    </div>
    ...
    <script type="text/javascript">
        ...
        layui.use(['laydate', 'laypage','table','form'], function(){
          ...
          productListTalbe = table.render({
            elem: '#product_list'
            ,height: "full-215"
            ,url: '?m=system&s=users&a=get_operate_jilu&userId=<?=$id?>'
            ,page: {curr:<?=$page?>}
            ,limit:<?=$limit?>
            ,cols: [[<?=$rowsJS?>]]
            ,where:{
