...
$allRows = array(
				"area"=>array("title"=>"地区","rowCode"=>"{field:'area',title:'地区',width:200}"),
                // "channelTitle"=>array("title"=>"分类","rowCode"=>"{field:'channelTitle',title:'分类',width:120}"),
				"name"=>array("title"=>"姓名","rowCode"=>"{field:'name',title:'姓名',width:180}"),
				"phone"=>array("title"=>"电话","rowCode"=>"{field:'phone',title:'电话',width:180}"),
				"email"=>array("title"=>"邮箱","rowCode"=>"{field:'email',title:'邮箱',width:180}"),
				"move"=>array("title"=>"移动","rowCode"=>"{field:'move',title:'移动',width:150}"),
				"dtTime"=>array("title"=>"添加日期","rowCode"=>"{field:'dtTime',title:'添加日期',width:180,sort:true}"),
			);
...
$channels = array();
$channels = $db->get_results("select id,title from demo_skill_channel where comId=$comId and parentId = 0 order by ordering desc,id asc");

foreach ($channels as $k => $channel){
    $channel->channels = [];
    $childs = $db->get_results("select id,title from demo_skill_channel where comId=$comId and parentId = $channel->id order by ordering desc,id asc");
    if($childs){
        foreach ($childs as $ck => $child){
            $childs[$ck]->channels = [];
            $childd = $db->get_results("select id,title from demo_skill_channel where comId=$comId and parentId = $child->id order by ordering desc,id asc"); 
            if($childd){
...
<body>
	<div class="right_up">
		<img src="images/biao_19.png"/> 销售人员列表
	</div>
	<div class="right_down" style="padding-bottom:0px;">
     ...
     <!--               	<input type="checkbox" name="hebing" lay-filter="hebing" lay-skin="primary" checked="true" title="将招聘信息合并">-->
     <!--               </div>-->
					<div class="splist_up_01_right">	
						<div class="splist_up_01_right_1">
							<div class="splist_up_01_right_1_left">
								<input type="text" id="keyword" value="<?=$keyword?>" placeholder="请输入地区/姓名/手机号"/>
							</div>
											...
											<ul>
												<li>
													...
													<div class="gaojisousuo_right">
														<input type="text" name="super_keyword" class="gaojisousuo_right_input" placeholder="请输入招聘信息名称/编码/规格/关键字/条形码"/>
													</div>
													<div class="gaojisousuo_left">
														招聘信息分类
													</div>
						...
						<div class="splist_up_01_right_3">
						    <? chekurl($arr,'<a href="?&m=system&s=users&a=add_salesman" class="splist_add">新 增</a>') ?>
						</div>
						<div class="clearBoth"></div>
					</div>
					<div class="clearBoth"></div>
			...
			<div class="splist_down1">
				...
				<div class="yuandian_xx" id="operate_row" data-id="0">
					<ul>
						<? chekurl($arr,'<li><a href="javascript:" _href="?&m=system&s=users&a=add_salesman" onclick="edit_anli()"><img src="images/biao_31.png"> 编辑</a></li>') ?>
			            <?chekurl($arr,'<li><a href="javascript:" _href="?m=system&s=skill&a=process" onclick="bind_gift()"><img src="images/biao_117.png">流程</a></li>') ?>
			            <?chekurl($arr,'<li><a href="javascript:" _href="?m=system&s=skill&a=fangan" onclick="showCard()"><img src="images/biao_96.png">方案</a></li>') ?>
			            <?chekurl($arr,'<li><a href="javascript:" _href="?m=system&s=skill&a=anli" onclick="showAnli()"><img src="images/biao_30.png">案例</a></li>') ?>
						<? chekurl($arr,'<li><a href="javascript:" _href="?m=system&s=users&a=delSalesman" onclick="z_confirm(\'确定要删除该销售人员信息吗？\',del_anli,\'\');"><img src="images/biao_32.png"> 删除</a></li>') ?>
					</ul>
				</div>
	...
	<script type="text/javascript">
		...
		layui.use(['laydate', 'laypage','table','form'], function(){
		  ...
		  ,active = {
		  	setTags:function(){
				layer.open({
					...
					'<div class="spxx_shanchu_tanchu_01_left">'+'设置招聘信息标签'+
					'</div>'+
		  ...
		  productListTalbe = table.render({
		    elem: '#product_list'
		    ,height: "full-140"
		    ,url: '?m=system&s=users&a=getSalesmanList'
		    ,page: {curr:<?=$page?>}
		    ,limit:<?=$limit?>
		    ,cols: [[<?=$rowsJS?>]]
		    ,where:{
		...
		function edit_anli(params){
        	...
        	var url = '?m=system&s=users&a=salesman&channelId='+channelId+"&brandId="+brandId+"&status="+status+"&keyword="+keyword+"&tags="+tags+"&source="+source+"&cuxiao="+cuxiao+"&page="+page+"&order1="+order1+"&order2="+order2;
        	url = encodeURIComponent(url);
        	location.href="?&m=system&s=users&a=add_salesman&id="+pdtId+"&url="+url;
        }
        
        function showAnli(params)
        {
            if(params>0){
        		pdtId = params;
        	}else{
        	...
        	var order1 = $("#order1").val();
        	var order2 = $("#order2").val();
        	var url = '?m=system&s=skill&channelId='+channelId+"&brandId="+brandId+"&status="+status+"&keyword="+keyword+"&tags="+tags+"&source="+source+"&cuxiao="+cuxiao+"&page="+page+"&order1="+order1+"&order2="+order2;
        	url = encodeURIComponent(url);
        	
        	location.href="?m=system&s=skill&a=anli&skillId="+pdtId+"&url="+url;
        }
        ...
        function showCard(params)
        {
        	...
        	location.href="?m=system&s=skill&a=fangan&skillId="+pdtId+"&url="+url;
        }
        ...
        function bind_gift(params){
        	...
        	location.href="?m=system&s=skill&a=process&skillId="+pdtId+"&url="+url;
        }
        ...
        function del_anli(params){
        	...
        	ajaxpost=$.ajax({
        		type: "POST",
        		url: "?m=system&s=users&a=delSalesman",
        		data: "&ids="+pdtId,
        		dataType:"json",timeout : 8000,
        		success: function(resdata){
        			if(resdata.code==1){
...
function moveSaleman(id, state)
{
    var tip = '确定要上移当前销售人员吗？';
    if(state == 0){
        tip = '确定要下移当前销售人员吗？';
    }
    layer.confirm(tip, {
	  btn: ['确定','取消'],
		...
		ajaxpost=$.ajax({
			type: "POST",
			url: "?m=system&s=users&a=moveSalesman",
...
function delChange(){
	...
	layer.confirm('确定要删除选中的'+num+'个招聘信息吗？', {
	  btn: ['确定','取消'],
	},function(){
		layer.closeAll();
		layer.load();
