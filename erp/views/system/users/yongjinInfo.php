...
$allRows = array(
    ...
    "yue"=>array("title"=>"账户余额","rowCode"=>"{field:'yue',title:'账户余额',width:100}"),
    "dtTime"=>array("title"=>"操作时间","rowCode"=>"{field:'dtTime',title:'操作时间',width:150}"),
    "remark"=>array("title"=>"类型","rowCode"=>"{field:'remark',title:'类型',width:200}"),
...
$id = (int)$request['id'];
$user = $db->get_row("select nickname,money,yongjin,yongjins from users where id=$id and comId=$comId");
$liushuis = $db->get_results("select type,sum(money) as money from user_liushui$fenbiao where comId=$comId and userId=$id group by type");
$tixian_money = 0;
$xiaofei_money = 0;
$chongzhi_money = 0;
if(!empty($liushuis)){
...
//0-商品分润 1-业绩达成 2-门店以上月分润 3-提现  4-取消订单返还佣金   5-下订单
$chongzhi_money = $db->get_var("select sum(money) from user_yongjin8 where comId=$comId and userId=$id and type in (0,1,2)");

$xiaofei_money = $db->get_var("select sum(money) from user_yongjin8 where comId=$comId and userId=$id and type in (3,4,5)");
...
$rols = explode('|', $arr);
$canChange = 0;
if(in_array('?m=system&s=users&a=chongzhi', $rols)){
    $canChange = 1;
}
...
<body>
    <div class="mendianguanli"> 
        <div class="mendianguanli_up">
            <a href="<?=urldecode($request['returnurl'])?>"><img src="images/users_39.png"></a> <b style="color:#369dd0;"><?=$user->nickname?></b> 佣金详情
        </div>
                ...
                <div class="huiyuanxinxi_down">
                    <div class="hyxx_yuemingxi_up">
                        <ul>
                            ...
                            <li style="background-color:#7acad1;" onclick="select_type(3);" onmouseenter="tips(this,'点击查看所有提现记录',1);" onmouseleave="hideTips();">
                                <div class="hyxx_yuemingxi_up_left">
                                    <h2>¥ <?=floatval($xiaofei_money)?></h2>总提现金额
                                </div>
                                <div class="clearBoth"></div>
                            </li>
                            <li style="background-color:#b19ecb;" onclick="select_type(1);" onmouseenter="tips(this,'点击查看所有消费记录',1);" onmouseleave="hideTips();">
                                <div class="hyxx_yuemingxi_up_left">
                                    <h2>¥ <?=floatval($chongzhi_money)?></h2>累计收入金额
                                </div>
                                <div class="clearBoth"></div>
                            </li>
                            <li style="background-color:#f98e88;" onclick="select_type(2);" onmouseenter="tips(this,'点击查看所有充值记录',1);" onmouseleave="hideTips();">                                
                                <div class="hyxx_yuemingxi_up_left">
                                    <h2>¥ <?=$user->yongjins?></h2>累计佣金金额
                                </div>
                                <div class="clearBoth"></div>
                            </li>
                            <div class="clearBoth"></div>
    ...
    <script type="text/javascript">
        ...
        layui.use(['laydate', 'laypage','table','form'], function(){
          ...
          productListTalbe = table.render({
            elem: '#product_list'
            ,height: "full-340"
            ,url: '?m=system&s=users&a=getYongJinList&userId=<?=$id?>'
            ,page: true
            ,limit:<?=$limit?>
            ,cols: [[<?=$rowsJS?>]]
