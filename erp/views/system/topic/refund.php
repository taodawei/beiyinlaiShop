<?
global $db,$request;
$type = (int)$request['agent_type'];

$shezhi = $db->get_row("select * from demo_shezhi where comId= 1207");
switch ($type) {
    case 4:
        $ruleJson = $shezhi->agent_rule;
        $title = '个人代理';
        break;
    case 5:
        $ruleJson = $shezhi->agent_mendian;
        $title = '门店代理';
        break;  
    case 1:
        $ruleJson = $shezhi->agent_city;
        $title = '市级代理';
        break;
    case 2:
        $ruleJson = $shezhi->agent_province;
        $title = '省级代理';
        break;
    case 3:
        $ruleJson = $shezhi->agent_area;
        $title = '区域代理';
        break;      
}
...
$list = $db->get_results("select * from demo_product_refund order by ordering desc ");
...
<body>
	<form action="?m=system&s=topic&a=refund&submit=1&type=<?=$type?>" method="post" id="createPdtForm" class="layui-form" enctype="multipart/form-data">
		<input type="hidden" name="url" value="<?=$url?>">
		<div class="content_edit">
			<div class="edit_h">
				<!--<a href="?m=system&s=mendian_set&a=agent"><img src="images/back.jpg" /></a>-->
				<span>售后反馈表设置</span>
			</div>
				
		<div class="edit_guige">
           	<div class="jichu_h"></div>
            <div class="">
                <table class="layui-table" id="serviceTables">
                    <colgroup>
                        ...
                        <tr>
                            ...
                            <th><span>下载链接</span><span class="layui-table-sort layui-inline" style="margin-top: 5px"></th>
                            <th>操作</th>
                        </tr>
                        
                    	<? if(!empty($list)){
							foreach ($list as $ck => $custom){
								?>
								<tr>
                                    <td>
                                        <input type="text" name="addRowKey[]" lay-verify="required" value="<?=$custom->title?>" autocomplete="off" class="layui-input">
                                    </td>
                                   <td>
                                       <input type="text" name="addRowValue[]"  lay-verify="required"  value="<?=$custom->download?>" autocomplete="off" class="layui-input">
                                   </td>
                                    <td>
                ...
                <button id="newBtn" type="button" class="layui-btn layui-btn-primary layui-btn-fluid"><i class="layui-icon"></i> 新增反馈表</button><br>
            </div>
        </div>
    ...
    layui.use(['upload','form','layer'], function(){
        ...
        $("#newBtn").click(function() {
            ...
            str += '<tr>';
            str += '<td>';
            str += '<input type="text" name="addRowKey[]" lay-verify="required" placeholder="请输入反馈表名称" autocomplete="off"';
            str += 'class="layui-input">';
            // str += '<?=$yhqSelect?>';
            str += '</td>';
            str += '<td><input type="text" name="addRowValue[]"  lay-verify="required"  placeholder="反馈表下载链接" autocomplete="off"';
        ...
        $("#serviceTables").on("click", ".bjBtn", function () {
            ...
            thinLine.each(function (i) {
				...
				<div class="edit_miaoshu" style="display:none;">
					<div class="miaoshu_tt">
						套餐描述<span style="color:#9b9b9b"></span>
					</div>
					<div class="miaoshu_fenlei" id="pdtcontMenu">
						<ul>
							<li><a href="javascript:" id="pdtcontMenu1" onclick="qiehuan('pdtcont',1,'on');" class="on">套餐描述</a></li>
						</ul>
					</div>
