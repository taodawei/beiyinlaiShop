...
$allRows = array(
				"logo"=>array("title"=>"充值卡图片","rowCode"=>"{field:'logo',title:'充值卡图片',width:150}"),
				"title"=>array("title"=>"充值卡名称","rowCode"=>"{field:'title',title:'充值卡名称',width:250}"),
				"price"=>array("title"=>"面值","rowCode"=>"{field:'price',title:'面值',width:150}"),
				// "change_num"=>array("title"=>"可兑/总商品","rowCode"=>"{field:'change_num',title:'可兑/总商品',width:150}"),
				// "change_num1"=>array("title"=>"预警下线商品数量","rowCode"=>"{field:'change_num1',title:'预警下线商品数量',width:150}"),
                "channelTitle"=>array("title"=>"分类","rowCode"=>"{field:'channelTitle',title:'分类',width:220}"),
                "beizhu"=>array("title"=>"备注","rowCode"=>"{field:'beizhu',title:'备注',width:280}"),
				"ordering"=>array("title"=>"排序","rowCode"=>"{field:'ordering',title:'排序',width:160,sort:true}")
			);
...
<body>
	<div class="right_up">
		<img src="images/biao_19.png"/> 充值卡列表
	</div>
	<div class="right_down" style="padding-bottom:0px;">
     ...
     <!--               	<input type="checkbox" name="hebing" lay-filter="hebing" lay-skin="primary" checked="true" title="将充值卡合并">-->
     <!--               </div>-->
					<div class="splist_up_01_right">	
						<div class="splist_up_01_right_1">
							<div class="splist_up_01_right_1_left">
								<input type="text" id="keyword" value="<?=$keyword?>" placeholder="请输入充值卡名称"/>
							</div>
											...
											<ul>
												<li>
													...
													<div class="gaojisousuo_right">
														<input type="text" name="super_keyword" class="gaojisousuo_right_input" placeholder="请输入充值卡名称/编码/规格/关键字/条形码"/>
													</div>
													<div class="gaojisousuo_left">
														充值卡分类
													</div>
						...
						<div class="splist_up_01_right_3">
						    <? chekurl($arr,'<a href="?&m=system&s=recharge&a=add" class="splist_add">新 增</a>') ?>
						</div>
						<div class="clearBoth"></div>
					</div>
					<div class="clearBoth"></div>
				...
				<div class="splist_up_02">
					...
					<div class="splist_up_02_3">
						...
						<? chekurl($arr,'<a href="javascript:" _href="?m=system&s=recharge&a=del" onclick="delreChange();"><img src="images/biao_28.png"/> 删除</a>') ?>
			...
			<div class="splist_down1">
				...
				<div class="yuandian_xx" id="operate_row" data-id="0">
					<ul>
						<? chekurl($arr,'<li><a href="javascript:" _href="?&m=system&s=recharge&a=add" onclick="edit_anli()"><img src="images/biao_31.png"> 编辑</a></li>') ?>
			            ...
			            <?chekurl($arr,'<li><a href="javascript:" _href="?m=system&s=recharge" onclick="showCard()"><img src="images/biao_96.png">卡列表</a></li>') ?>
			            
						<? chekurl($arr,'<li><a href="javascript:" _href="?&m=system&s=change&a=del" onclick="z_confirm(\'确定要删除该充值卡吗？\',del_anli,\'\');"><img src="images/biao_32.png"> 删除</a></li>') ?>
					</ul>
				</div>
	...
	<script type="text/javascript">
		...
		layui.use(['laydate', 'laypage','table','form'], function(){
		  ...
		  ,active = {
		  	setTags:function(){
				layer.open({
					...
					'<div class="spxx_shanchu_tanchu_01_left">'+'设置充值卡标签'+
					'</div>'+
		  ...
		  productListTalbe = table.render({
		    elem: '#product_list'
		    ,height: "full-140"
		    ,url: '?m=system&s=recharge&a=getCardList'
		    ,page: {curr:<?=$page?>}
		    ,limit:<?=$limit?>
		    ,cols: [[<?=$rowsJS?>]]
		    ,where:{
		...
		function edit_anli(params){
        	...
        	location.href="?m=system&s=recharge&a=add&id="+pdtId+"&url="+url;
        }
        ...
        function showCard(params)
        {
        	...
        	location.href="?m=system&s=recharge&id="+pdtId+"&url="+url;
        }
...
function delreChange(){
	var ids = $("#selectedIds").val();
	var num = $("#selectedNum").html();
	layer.confirm('确定要删除选中的'+num+'个充值卡吗？', {
	  btn: ['确定','取消'],
	},function(){
		layer.closeAll();
		layer.load();
