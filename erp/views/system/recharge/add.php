...
$caiwu = array();
if(!empty($id)){
    $kehu = $db->get_row("select * from kmd_recharge where id=$id");
...
<body>
	<div class="back1">
        ...
        <div><? if(empty($kehu)){?>添加<? }else{ echo '修改';}?>充值卡</div>
    </div>
    <div class="cont" style="height:725px;">
        <form action="?s=recharge&a=add&tijiao=1&id=<?=$id?>" method="post" id="submitForm" class="layui-form">
            ...
            <div class="provider_cont">
                ...
                <table width="100%" cellpadding="0" cellspacing="0">
                    <tr>
                         <td class="provider_td">
                            ...
                            <div class="cont_left_input1">
                                <input name="title" class="layui-input" type="text" value="<?=$kehu->title?>" lay-verify="required" placeholder="请输入可充值卡名称"/>
                            </div>
                         ...
                         <td class="provider_td">
                            ...
                            <div class="cont_left_input1">
                                <div class="layui-form-select" onmouseenter="tips(this,'请选择卡分类',1);" onmouseout="hideTips();">
									<div class="layui-select-title" id="selectChannel"><input type="text" readonly placeholder="请选择分类" value="<?=$db->get_var("select title from demo_recharge_channel where id=".(int)$kehu->channelId);?>" class="layui-input"><i class="layui-edge"></i></div>
                    ...
                    <tr>
                        <td class="provider_td">
                            <div class="cont_left_tt">
                                <span class="must">*</span>面值
                            </div>
                            <div class="cont_left_input1">
                                <input name="price" class="layui-input" type="number" min="0" step="1" value="<?=$kehu->price?>" lay-verify="required" min="0" step="1" placeholder="面值"/>
                            </div>
                            <div class="clearBoth"></div>
                        </td> 
                    </tr> 
                    ...
                    <tr>
                        <td class="provider_td">
                            ...
                            <div class="cont_left_input1">
                                <input name="ordering" class="layui-input" type="number" min="0" step="1" value="<?=$kehu->ordering?>" lay-verify="required" min="0" step="1" placeholder="面值"/>
                            </div>
                            <div class="clearBoth"></div>
                        </td> 
                    </tr> 
    ...
    <script type="text/javascript">
      ...
      layui.use(['laydate','form','upload'], function(){
        ...
        ajaxpost = $.ajax({
    		type: "POST",
    		url: "/erp_service.php?action=get_recharge_channels1",
    		data: "pid="+channelId,
