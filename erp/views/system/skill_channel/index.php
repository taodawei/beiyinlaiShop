...
// }
$channels = null;
if(empty($channels))$channels = $db->get_results("select * from demo_skill_channel where comId=$comId and parentId=0 order by ordering desc,id asc");
$showArry = array();
$id = (int)$request['id'];
if(!empty($id)){
...
<head>
	...
	<script type="text/javascript" src="js/skill_channel.js?v=1"></script>
</head>
<body>
	<div class="jiliangdanwei">
		<div class="jiliangdanwei_up">
			<div class="jiliangdanwei_up_left">
				<img src="images/biao_35.png"/> 招聘分类管理
			</div>
			<div class="jiliangdanwei_up_right">
			    <? chekurl($arr,'<a href="javascript:" _href="?m=system&s=skill_channel&a=addProductChannel" onclick="edit_channel(0,0,\'\',\'\',\'\',0,\'\');">+ 新 增</a>') ?>
			</div>
				...
				if(!empty($channels)){
					foreach ($channels as $c){
						...
						if($cache==1){
							...
							$channels1 = $db->get_results("select * from demo_skill_channel where comId=$comId and parentId=".$c->id." order by ordering desc,id asc");
						}
						if(!empty($channels1)){
							$ifdele = 0;
						}else{
						...
						<li data-id="<?=$c->id?>" data-pid="0">
							<div class="shangpinguanli_01">
								...
								<div class="shangpinguanli_01_right">
									...
									<? chekurl($arr,'<a href="javascript:" _href="?m=system&s=skill_channel&a=addProductChannel" onclick="edit_channel('.$c->id.',0,\''.$c->title.'\',\''.$c->en_title.'\',\''.$c->originalPic.'\',\''.$c->backimg.'\',\''.$c->is_hot.'\',\''.$c->type.'\',\''.$c->remark.'\');"><img src="images/biao_49.png"/> 修改</a>') ?>
									...
									<ul>
										...
										foreach ($channels1 as $c1) {
											...
											if($cache==1){
												...
												$channels2 = $db->get_results("select * from demo_skill_channel where comId=$comId and parentId=".$c1->id." order by ordering desc,id asc");
											}
											if(!empty($channels2)){
												$ifdele = 0;
											}else{
