...
$allRows = array(
				"title"=>array("title"=>"岗位名称","rowCode"=>"{field:'title',title:'岗位名称',width:150}"),
				...
				"language"=>array("title"=>"语言","rowCode"=>"{field:'language',title:'语言',width:150}"),
...
$channels = array();
$channels = $db->get_results("select id,title from demo_recruit_channel where comId=$comId and parentId = 0 order by ordering desc,id asc");

foreach ($channels as $k => $channel){
    $channel->channels = [];
    $childs = $db->get_results("select id,title from demo_recruit_channel where comId=$comId and parentId = $channel->id order by ordering desc,id asc");
    if($childs){
        foreach ($childs as $ck => $child){
            $childs[$ck]->channels = [];
            $childd = $db->get_results("select id,title from demo_recruit_channel where comId=$comId and parentId = $child->id order by ordering desc,id asc"); 
            if($childd){
...
<body>
	<div class="right_up">
		<img src="images/biao_19.png"/> 人才招聘列表
	</div>
     ...
     <!--               </div>-->
					<div class="splist_up_01_right">	
						<div class="splist_up_01_right_1">
							<div class="splist_up_01_right_1_left">
								<input type="text" id="keyword" value="<?=$keyword?>" placeholder="请输入招聘信息名称"/>
							</div>
						...
						<div class="splist_up_01_right_3">
						    <? chekurl($arr,'<a href="?&m=system&s=recruit&a=add" class="splist_add">新 增</a>') ?>
						</div>
						<div class="clearBoth"></div>
					</div>
					<div class="clearBoth"></div>
			...
			<div class="splist_down1">
				...
				<div class="yuandian_xx" id="operate_row" data-id="0">
					<ul>
						<? chekurl($arr,'<li><a href="javascript:" _href="?&m=system&s=recruit&a=add" onclick="edit_anli()"><img src="images/biao_31.png"> 编辑</a></li>') ?>
						...
						<? chekurl($arr,'<li><a href="javascript:" _href="?&m=system&s=recruit&a=del" onclick="z_confirm(\'确定要删除该招聘信息吗？\',del_anli,\'\');"><img src="images/biao_32.png"> 删除</a></li>') ?>
					</ul>
				</div>
	...
	<script type="text/javascript">
		...
		layui.use(['laydate', 'laypage','table','form'], function(){
		  ...
		  productListTalbe = table.render({
		    elem: '#product_list'
		    ,height: "full-140"
		    ,url: '?m=system&s=recruit&a=getList'
		    ,page: {curr:<?=$page?>}
		    ,limit:<?=$limit?>
		    ,cols: [[<?=$rowsJS?>]]
		    ,where:{
		...
		function edit_anli(params){
        	...
        	var url = '?m=system&s=recruit&a=index&channelId='+channelId+"&brandId="+brandId+"&status="+status+"&keyword="+keyword+"&tags="+tags+"&source="+source+"&cuxiao="+cuxiao+"&page="+page+"&order1="+order1+"&order2="+order2;
        	url = encodeURIComponent(url);
        	location.href="?m=system&s=recruit&a=add&id="+pdtId+"&url="+url;
        }
