...
$allRows = array(
    ...
    "fahuoTime"=>array("title"=>"发货时间","rowCode"=>"{field:'fahuoTime',title:'发货时间',width:159}"),
...
$num_sql = "select kuaidi_type,count(*) as num from order_fahuo$fenbiao where comId=$comId and yushouId=$yushouId and status=3";
$yushou = $db->get_row("select title from demo_product_inventory where id=(select pdtId from yushou where id=$yushouId) ");
$title = $yushou->title.' -预售- ';
$title .= '发货成功';
$zongNum = 0;
$numArry = array();
$num_sql.=" group by kuaidi_type";
$nums = $db->get_results($num_sql);
if(!empty($nums)){
    foreach ($nums as $n){
        $zongNum+=$n->num;
        $numArry[$n->kuaidi_type] = $n->num;
    }
}
?>
        ...
        <div class="splist">
            <div class="splist_up" style="height:118px;">
                <div class="splist_up_addtab">
                    <ul>
                        <li>
                            <a href="?s=yushou&a=fhsuccess&yushouId=<?=$request['yushouId']?>" <? if(empty($type)){?>class="splist_up_addtab_on"<? }?>>全部(<?=$zongNum?>)</a>
                        </li>
                        <li>
                            <a href="?s=yushou&type=1&a=fhsuccess&yushouId=<?=$request['yushouId']?>" <? if($type==1){?>class="splist_up_addtab_on"<? }?>>普通发货(<?=(int)$numArry[1]?>)</a>
                        </li>
                        <li>
                            <a href="?s=yushou&type=2&a=fhsuccess&yushouId=<?=$request['yushouId']?>" <? if($type==2){?>class="splist_up_addtab_on"<? }?>>电子面单发货(<?=(int)$numArry[2]?>)</a>
                        </li>
                    ...
                    <div class="splist_up_01_right">
                        <div class="splist_up_01_right_1">
                            <div class="splist_up_01_right_1_left">
                                <input type="text" id="keyword" value="<?=$keyword?>" placeholder="发货单号/订单号/收货人信息/商品名称/快递单号"/>
                            </div>
