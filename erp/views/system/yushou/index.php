...
$yushou = $db->get_row("select title from demo_product_inventory where id=(select pdtId from yushou where id=$yushouId) ");
$num_sql = "select status,count(*) as num from order_fahuo$fenbiao where comId=$comId and yushouId=$yushouId and status>-1 and (status=0 or kuaidi_type=2)";
/*if($_SESSION[TB_PREFIX.'admin_roleId']<7){
    $num_sql.=" and storeId in(".$_SESSION[TB_PREFIX.'quanxian']['kucun']['storeIds'].")";
}*/
$title = $yushou->title.' -预售- 电子面单打印';
...
<body>
<? require('views/system/yushou/header.php')?>
<div id="content">
    <div class="right_up">
        ...
        <div class="splist">
            <div class="splist_up" style="height:118px;">
                <div class="splist_up_addtab">
                    <ul>
                        <li>
                            <a href="?s=yushou&type=0&scene=<?=$scene?>&yushouId=<?=$request['yushouId']?>" <? if(empty($type)){?>class="splist_up_addtab_on"<? }?>>全部(<?=$zongNum?>)</a>
                        </li>
                        <li>
                            <a href="?s=yushou&type=1&scene=<?=$scene?>&yushouId=<?=$request['yushouId']?>" <? if($type==1){?>class="splist_up_addtab_on"<? }?>>待打印(<?=(int)$numArry[0]?>)</a>
                        </li>
                        <li>
                            <a href="?s=yushou&type=2&scene=<?=$scene?>&yushouId=<?=$request['yushouId']?>" <? if($type==2){?>class="splist_up_addtab_on"<? }?>>待确认(<?=(int)$numArry[1]?>)</a>
                        </li>
                        <li>
                            <a href="?s=yushou&type=3&scene=<?=$scene?>&yushouId=<?=$request['yushouId']?>" <? if($type==3){?>class="splist_up_addtab_on"<? }?>>已确认(<?=(int)$numArry[3]?>)</a>
                        </li>
                ...
                <div class="splist_up_02">
                    ...
                    <div class="dangqiandd_2_down_3">
                        <a href="javascript:;" id="print_fahuo"><img src="images/miandan_1.png" /> 打印发货单</a>
                        <a href="javascript:;" id="zanting_fahuo"><img src="images/order/dangqiandingdan_14.png" /> 暂停发货</a>
                    </div>
                    <? }?>
                    <div class="clearBoth"></div>
                </div>
...
<div class="damx_genghuanwuliu_tc" style="display:none;">
    ...
    <div class="damx_genghuanwuliu">
        <div class="damx_genghuanwuliu_1">
            ...
            <div class="damx_genghuanwuliu_1_right" onclick="$('.damx_genghuanwuliu_tc').hide();">
                    ...
                    if($storeId>0){
                        ...
                        if(!empty($kuaidis)){
                            foreach ($kuaidis as $kuaidi) {
                                ?>
                                <option value="<?=$kuaidi->kuaidi_company?>" data-value="<?=$kuaidi->id?>"><?=$kuaidi->kuaidi_title?></option>
                                <?
                            }
                        }
                    }
...
<div class="print_tc" style="display:none;">
    ...
    <div class="damx_genghuanwuliu">
        ...
        <div style="text-align: center;display: none;"><iframe src="https://www.kdniao.com/External/GetIp.aspx" id="frame1" style="width:200px;height:30px;overflow:hidden;border:0px;"></iframe></div>
        <div class="damx_genghuanwuliu_2" style="text-align: center;">
            <input type="text" id="kuaidi_ip" name="kuaidi_ip" value="" style="width: 75%;height: 35px;margin-bottom: 10px;display: none;" placeholder="请复制上面显示ip">
            <div class="damx_genghuanwuliu_2_2" id="shuju">
                成功：0条，失败：0条
            </div>
        </div>
        <div class="damx_genghuanwuliu_3">
            <a href="javascript:;" id="prints" target="_blank" onclick="return xiaoshi();">打印</a>
        </div>
    </div>
</div>
<!--修改收货信息-->
...
<input type="hidden" id="yushouId" value="<?=$yushouId?>">
<script type="text/javascript">
    function xiaoshi(){
        /*var kuaidi_ip = $("#kuaidi_ip").val();
        if(kuaidi_ip==''){
            $("#kuaidi_ip").focus();
            return false;
        }else{
            var hrefs = $("#prints").attr("href");
            $("#prints").attr("href",hrefs+"&kuaidi_ip="+kuaidi_ip);
        }*/
        $(".print_tc").hide();
        reloadTable(0);
    }
...
<script type="text/javascript" src="js/yushou/dianzi_list.js"></script>
<script type="text/javascript" src="js/yushou/dianzi_info.js"></script>
