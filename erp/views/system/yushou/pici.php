...
$num_sql = "select count(*) as num,type from fahuo_pici$fenbiao where comId=$comId and yushouId=$yushouId and (type=2 or need_peihuo=0) group by type";
$title = '发货批次';
$zongNum = 0;
$numArry = array();
//$num_sql.=" group by id";
        ...
        <div class="splist">
            <div class="splist_up" style="height:118px;">
                <div class="splist_up_addtab">
                    <ul>
                        <li>
                            <a href="?s=yushou&type=0&a=pici&yushouId=<?=$yushouId?>" <? if($type==0){?>class="splist_up_addtab_on"<? }?>>全部(<?=$zongNum?>)</a>
                        </li>
                        <li>
                            <a href="?s=yushou&type=1&a=pici&yushouId=<?=$yushouId?>" <? if($type==1){?>class="splist_up_addtab_on"<? }?>>普通发货(<?=(int)$numArry[1]?>)</a>
                        </li>
                        <li>
                            <a href="?s=yushou&type=2&a=pici&yushouId=<?=$yushouId?>" <? if($type==2){?>class="splist_up_addtab_on"<? }?>>电子面单(<?=(int)$numArry[2]?>)</a>
                        </li>
...
<script type="text/javascript">
    ...
    layui.use(['laydate', 'laypage','table','form'], function(){
      ...
      productListTalbe = table.render({
        elem: '#product_list'
        ,height: "full-200"
        ,url: '?s=fahuo&a=getPiciLists&yushouId=<?=$yushouId?>'
        ,page: {curr:<?=$page?>}
        ,limit:<?=$limit?>
        ,cols: [[<?=$rowsJS?>]]
        ,where:{
