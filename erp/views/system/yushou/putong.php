...
//计算各类型订单的数量
$num_sql = "select status,count(*) as num from order_fahuo$fenbiao where comId=$comId and yushouId=$yushouId and status>-1 and (status=0 or print_type=1) ";
		...
		<div class="splist">
			<div class="splist_up" style="height:118px;">
				<div class="splist_up_addtab">
	            	<ul>
	            		<li>
	                    	<a href="?s=yushou&a=putong&scene=<?=$scene?>&yushouId=<?=$request['yushouId']?>" <? if(empty($type)){?>class="splist_up_addtab_on"<? }?>>全部(<?=$zongNum?>)</a>
	                    </li>
	                    <li>
	                    	<a href="?s=yushou&type=1&a=putong&scene=<?=$scene?>&yushouId=<?=$request['yushouId']?>" <? if($type==1){?>class="splist_up_addtab_on"<? }?>>未导出(<?=(int)$numArry[0]?>)</a>
	                    </li>
	                    <li>
	                    	<a href="?s=yushou&type=2&a=putong&scene=<?=$scene?>&yushouId=<?=$request['yushouId']?>" <? if($type==2){?>class="splist_up_addtab_on"<? }?>>待导入(<?=(int)$numArry[1]?>)</a>
	                    </li>
	                    <li>
	                    	<a href="?s=yushou&type=3&a=putong&scene=<?=$scene?>&yushouId=<?=$request['yushouId']?>" <? if($type==3){?>class="splist_up_addtab_on"<? }?>>已完成(<?=(int)$numArry[3]?>)</a>
	                    </li>
...
<script type="text/javascript" src="js/yushou/order_list.js"></script>
<script type="text/javascript" src="js/yushou/order_info.js"></script>
