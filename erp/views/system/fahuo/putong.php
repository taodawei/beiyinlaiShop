...
$allRows = array(
	"view"=>array("title"=>"发货","rowCode"=>"{field:'view',title:'发货',width:44,fixed: 'left'}"),
	...
	"daochutype"=>array("title"=>"导出状态","rowCode"=>"{field:'daochutype',title:'导出状态',width:105}"),
);
...
//计算各类型订单的数量
$num_sql = "select status,count(*) as num from order_fahuo$fenbiao where comId=$comId and status>-1 and status <> 3 and (status=0 or print_type=1) ";
$weixinchuzhi = "select count(*) as num from order_fahuo$fenbiao where comId=$comId and status>-1 and status <> 3 and (status=0 or print_type=1) ";

$mendianId = isset($request['mendianId']) ? $request['mendianId'] : $_SESSION['mendianId'];
if($mendianId > 0){
    $num_sql .= " and mendianId = $mendianId ";
    $weixinchuzhi .= " and mendianId = $mendianId ";
}

$weixin1 = $db->get_var($weixinchuzhi." and find_in_set(1, pay_types) ");
$weixin2 = $db->get_var($weixinchuzhi." and find_in_set(2, pay_types) ");
$weixin3 = $db->get_var($weixinchuzhi." and find_in_set(3, pay_types) ");
		...
		<div class="splist">
			<div class="splist_up" style="height:118px;">
				<div class="splist_up_addtab">
	            	<ul>
	            	    <!--字符串：0-未支付  1-微信储值 2-储值卡  3-微信支付-->
	            		<li>
	                    	<a href="?s=fahuo&a=putong&scene=0" <? if(empty($type) && $scene == 0){?>class="splist_up_addtab_on"<? }?>>全部(<?=$zongNum?>)</a>
	                    </li>
	                    <li>
	                    	<a href="?s=fahuo&a=putong&scene=1" <? if($scene==1){?>class="splist_up_addtab_on"<? }?>>微信储值(<?=$weixin1?>)</a>
	                    </li>
	                    <li>
	                    	<a href="?s=fahuo&a=putong&scene=2" <? if($scene==2){?>class="splist_up_addtab_on"<? }?>>储值卡(<?=$weixin2?>)</a>
	                    </li>
	                    <li>
	                    	<a href="?s=fahuo&a=putong&scene=3" <? if($scene==3){?>class="splist_up_addtab_on"<? }?>>微信支付(<?=$weixin3?>)</a>
	                    </li>
	                    <!--<li>-->
	                    <!--	<a href="?s=fahuo&type=1&a=putong&scene=<?=$scene?>" <? if($type==1){?>class="splist_up_addtab_on"<? }?>>未导出(<?=(int)$numArry[0]?>)</a>-->
	                    <!--</li>-->
	                    <!--<li>-->
	                    <!--	<a href="?s=fahuo&type=2&a=putong&scene=<?=$scene?>" <? if($type==2){?>class="splist_up_addtab_on"<? }?>>待导入(<?=(int)$numArry[1]?>)</a>-->
	                    <!--</li>-->
	                    <li>
	                    	<!--<a href="?s=fahuo&type=3&a=putong&scene=<?=$scene?>" <? if($type==3){?>class="splist_up_addtab_on"<? }?>>已完成(<?=(int)$numArry[3]?>)</a>-->
	                    </li>
				...
				<div class="splist_up_02">
					...
					<div class="dangqiandd_2_down_3">
					    <? chekurl($arr,'<a href="?m=system&s=fahuo&a=daochu" onclick="daochu();" id="daochu" target="_blank"><img src="images/dangqiandingdan_12.png"> 批量导出</a>') ?>
					</div>
					<div class="clearBoth"></div>
				</div>
			</div>
        	...
        	<div class="putongfh_fahuo_02_up">
            	<ul>
                    ...
                    <li>
                    	...
                    	<div class="putongfh_fahuo_02_up_right">
                        	<select id="rider_id" name="kuaidi_company" lay-search >
                        		<option value="" >选择物流公司</option>
                        		<? if(!empty($kuaidis)){
                        			foreach ($kuaidis as $kuaidi) {
                        				?><option value="<?=$kuaidi->kuaidi_company?>"  ><?=$kuaidi->kuaidi_company?></option><?
                        			}
                        		}?>
                        	</select>
                        </div>
...
<script type="text/javascript">
	...
	layui.use(['laydate', 'laypage','table','form'], function(){
	  ...
	  productListTalbe = table.render({
	    elem: '#product_list'
	    ,height: "full-200"
	    ,url: '?s=fahuo&a=getList&print_type=1'
	    ,page: {curr:<?=$page?>}
	    ,limit:<?=$limit?>
	    ,cols: [[<?=$rowsJS?>]]
	    ,where:{
