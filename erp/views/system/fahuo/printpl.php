...
$ids = $request['ids'];
$orders1 = $db->get_results("select * from order_fahuo$fenbiao where id in($ids) and comId=$comId");
if(empty($orders1))die("订单不存在！！");
foreach($orders1 as &$order){
    if(!empty($order->shuohuo_json))$order->shuohuo_json = json_decode($order->shuohuo_json,true);
...
<body>
	...
	<div class="right_down">
	    <? foreach($orders1 as $ordera){ ?>
		<div class="sprukuxiangxi" style="page-break-after: always;">
			<div class="sprukuxiangxi_01">	
				<div class="sprukuxiangxi_01_left">
						<span>发货单号：<?=$ordera->orderId?></span>
						<span>订单号：<?=$ordera->dingdan?></span><br>
						<span>成单时间：<?=date("Y-m-d H:i",strtotime($ordera->dtTime))?></span>
						<span>收货人：<?=$ordera->shuohuo_json["收件人"]?></span>
						<span>手机号：<?=$ordera->shuohuo_json["手机号"]?></span><br>
						<span>收货地区：<?=$ordera->shuohuo_json["所在地区"]?></span>
						<span>详细地址：<?=$ordera->shuohuo_json["详细地址"]?></span><br>
						<span>备注：<?=$ordera->remark?></span>
				</div>
			...
			<div class="sprukuxiangxi_02">
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
					...
					<?
					foreach($ordera->products as $k=>$v){
							?>
							<tr height="53">
