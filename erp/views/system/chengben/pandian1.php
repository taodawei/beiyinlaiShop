...
<head>
	...
	<script type="text/javascript">
		layui.use(['layer'], function(){
			...
			<? }else if(count($pandians[0])!=4){
				$confirm = 0;
				?>
				layer.confirm('数据格式有误，请先下载数据文件，修改好“成本调整”字段后上传', {
					btn: ['确定'],
				}, function(){
					location.href='?m=system&s=chengben&a=daoru&storeId=<?=$storeId?>';
				});
				...
				foreach ($pandians as $data) {
					if(!is_numeric($data[3])){
						$confirm = 0;
						?>
						layer.confirm('数据格式有误，请确保每一条数据都填写了“成本调整”', {
							btn: ['确定'],
						}, function(){
							location.href='?m=system&s=chengben&a=daoru&storeId=<?=$storeId?>';
						});
    	...
    	<div class="kucunpandian">
        	...
        	<div class="kucunpandian_yulan">
            	<div class="kucunpandian_yulan_01">
                	前 <?=$num?> 条数据预览  &nbsp;&nbsp;      成本调整仓库：<?=$db->get_var("select title from demo_kucun_store where id=$storeId")?>  &nbsp;&nbsp;    调整时间：<?=date("Y-m-d H:i")?>    &nbsp;&nbsp;   制表人：<?=$_SESSION[TB_PREFIX.'name']?> 
                </div>
        	...
        	<div class="kucunpandian_03">
        		<? if($confirm==1){?>
        		<form id="pandianForm" action="?m=system&s=chengben&a=pandian2&storeId=<?=$storeId?>" method="post">
