...
$allRows = array(
	...
	"orderId"=>array("title"=>"发货单号","rowCode"=>"{field:'orderId',title:'发货单号',width:200,style:\"height:auto;line-height:22px;white-space:normal;\"}"),
...
//计算各类型订单的数量
$num_sql = "select status,count(*) as num from kmd_change_log where 1=1 and status = 0 ";
		...
		<div class="splist">
			<div class="splist_up" style="height:118px;">
				<div class="splist_up_addtab">
	            	<ul>
	            		<li>
	                    	<a href="?s=change_log&scene=<?=$scene?>" <? if(empty($type)){?>class="splist_up_addtab_on"<? }?>>全部(<?=$zongNum?>)</a>
	                    </li>
	                    <!--<li>-->
					...
					<div class="splist_up_01_right">
						<div class="splist_up_01_right_1">
							<div class="splist_up_01_right_1_left">
								<input type="text" id="keyword" value="<?=$keyword?>" placeholder="发货单号/收货人信息/商品名称/兑换卡号"/>
							</div>
				...
				<div class="splist_up_02">
					...
					<div class="dangqiandd_2_down_3">
					    <? chekurl($arr,'<a href="?m=system&s=change_log&a=daochu" onclick="daochu();" id="daochu" target="_blank"><img src="images/dangqiandingdan_12.png"> 批量导出</a>') ?>
					</div>
					<div class="clearBoth"></div>
				</div>
			</div>
...
<script type="text/javascript">
	...
	layui.use(['form','laydate'], function(){
	  ...
	  form.on('submit(formDemo)', function(data){
	    ...
	    ajaxpost=$.ajax({
			type: "POST",
			url: "?s=change_log&a=order_fahuo",
			data: content,
		    dataType : "json",
		    timeout : 20000,
	...
	layui.use(['laydate', 'laypage','table','form'], function(){
	  ...
	  productListTalbe = table.render({
	    elem: '#product_list'
	    ,height: "full-200"
	    ,url: '?s=change_log&a=getList&print_type=1'
	    ,page: {curr:<?=$page?>}
	    ,limit:<?=$limit?>
	    ,cols: [[<?=$rowsJS?>]]
	    ,where:{
...
<script type="text/javascript" src="js/change_log/order_list.js"></script>
<script type="text/javascript" src="js/change_log/order_info.js"></script>
