...
//计算各类型订单的数量
$num_sql = "select kuaidi_type,count(*) as num from pdt_order_fahuo where comId=$comId and status=3";
$title = '发货成功';
$zongNum = 0;
$numArry = array();
        ...
        <div class="splist">
            <div class="splist_up" style="height:118px;">
                <div class="splist_up_addtab">
                    <ul>
                        <li>
                            <a href="?s=pdts_fahuo&a=fhsuccess" <? if(empty($type)){?>class="splist_up_addtab_on"<? }?>>全部(<?=$zongNum?>)</a>
                        </li>
                        <li>
                            <a href="?s=pdts_fahuo&type=1&a=fhsuccess" <? if($type==1){?>class="splist_up_addtab_on"<? }?>>普通发货(<?=(int)$numArry[1]?>)</a>
                        </li>
                        <li>
                            <a href="?s=pdts_fahuo&type=2&a=fhsuccess" <? if($type==2){?>class="splist_up_addtab_on"<? }?>>电子面单发货(<?=(int)$numArry[2]?>)</a>
                        </li>
...
<script type="text/javascript">
    ...
    layui.use(['laydate', 'laypage','table','form'], function(){
      ...
      productListTalbe = table.render({
        elem: '#product_list'
        ,height: "full-200"
        ,url: '?s=pdts_fahuo&a=getfhsuccess'
        ,page: {curr:<?=$page?>}
        ,limit:<?=$limit?>
        ,cols: [[<?=$rowsJS?>]]
        ,where:{
