...
//计算各类型订单的数量
$num_sql = "select status,count(*) as num from pdt_order_fahuo where comId=$comId  and status>-1 and (status=0 or kuaidi_type=2)";
        ...
        <div class="splist">
            <div class="splist_up" style="height:118px;">
                <div class="splist_up_addtab">
                    <ul>
                        <li>
                            <a href="?s=pdts_fahuo&type=0&scene=<?=$scene?>" <? if(empty($type)){?>class="splist_up_addtab_on"<? }?>>全部(<?=$zongNum?>)</a>
                        </li>
                        <li>
                            <a href="?s=pdts_fahuo&type=1&scene=<?=$scene?>" <? if($type==1){?>class="splist_up_addtab_on"<? }?>>待打印(<?=(int)$numArry[0]?>)</a>
                        </li>
                        <li>
                            <a href="?s=pdts_fahuo&type=2&scene=<?=$scene?>" <? if($type==2){?>class="splist_up_addtab_on"<? }?>>待确认(<?=(int)$numArry[1]?>)</a>
                        </li>
                        <li>
                            <a href="?s=pdts_fahuo&type=3&scene=<?=$scene?>" <? if($type==3){?>class="splist_up_addtab_on"<? }?>>已确认(<?=(int)$numArry[3]?>)</a>
                        </li>
...
<script type="text/javascript">
    ...
    layui.use('form', function(){
      ...
      form.on('submit(formDemo)', function(data){
        ...
        ajaxpost=$.ajax({
            type: "POST",
            url: "?s=pdts_fahuo&a=order_fahuo",
            data: content,
            dataType : "json",
            timeout : 20000,
    ...
    layui.use(['laydate', 'laypage','table','form'], function(){
      ...
      productListTalbe = table.render({
        elem: '#product_list'
        ,height: "full-200"
        ,url: '?s=pdts_fahuo&a=getList&print_type=2'
        ,page: {curr:<?=$page?>}
        ,limit:<?=$limit?>
        ,limits:[10,20,30,40]
        ,cols: [[<?=$rowsJS?>]]
	...
	function update_shouhuo(){
		...
		$.ajax({
			type: "POST",
			url: "?m=system&s=pdts_fahuo&a=update_shouhuo&id="+id,
...
<script type="text/javascript" src="js/pdts_fahuo/dianzi_list.js"></script>
<script type="text/javascript" src="js/pdts_fahuo/dianzi_info.js"></script>
