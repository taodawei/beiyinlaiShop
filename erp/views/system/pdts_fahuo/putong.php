...
//计算各类型订单的数量
$num_sql = "select status,count(*) as num from pdt_order_fahuo where comId=$comId and status>-1 and (status=0 or print_type=1) ";
$title = '普通快递发货';
$zongNum = 0;
$numArry = array();
		...
		<div class="splist">
			<div class="splist_up" style="height:118px;">
				<div class="splist_up_addtab">
	            	<ul>
	            		<li>
	                    	<a href="?s=pdts_fahuo&a=putong&scene=<?=$scene?>" <? if(empty($type)){?>class="splist_up_addtab_on"<? }?>>全部(<?=$zongNum?>)</a>
	                    </li>
	                    <li>
	                    	<a href="?s=pdts_fahuo&type=1&a=putong&scene=<?=$scene?>" <? if($type==1){?>class="splist_up_addtab_on"<? }?>>未导出(<?=(int)$numArry[0]?>)</a>
	                    </li>
	                    <li>
	                    	<a href="?s=pdts_fahuo&type=2&a=putong&scene=<?=$scene?>" <? if($type==2){?>class="splist_up_addtab_on"<? }?>>待导入(<?=(int)$numArry[1]?>)</a>
	                    </li>
	                    <li>
	                    	<a href="?s=pdts_fahuo&type=3&a=putong&scene=<?=$scene?>" <? if($type==3){?>class="splist_up_addtab_on"<? }?>>已完成(<?=(int)$numArry[3]?>)</a>
	                    </li>
				...
				<div class="splist_up_02">
					...
					<div class="dangqiandd_2_down_3">
						<a href="?s=pdts_fahuo&a=daochu" onclick="daochu();" id="daochu" target="_blank"><img src="images/dangqiandingdan_12.png"> 批量导出</a>
...
<script type="text/javascript">
	...
	layui.use(['laydate', 'laypage','table','form'], function(){
	  ...
	  productListTalbe = table.render({
	    elem: '#product_list'
	    ,height: "full-200"
	    ,url: '?s=pdts_fahuo&a=getList&print_type=1'
	    ,page: {curr:<?=$page?>}
	    ,limit:<?=$limit?>
	    ,cols: [[<?=$rowsJS?>]]
	    ,where:{
...
<script type="text/javascript" src="js/pdts_fahuo/order_list.js"></script>
<script type="text/javascript" src="js/pdts_fahuo/order_info.js"></script>
