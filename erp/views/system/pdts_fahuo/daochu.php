...
$ids = $request['ids'];
$sql="select * from pdt_order_fahuo where id in (".$ids.") and comId=$comId and status in(0,1)";
...
<table border="1" >   
	...
	<?
	if(!empty($pdts)){
		$db->query("update pdt_order_fahuo set status=1,print_type=1,kuaidi_type=1 where id in (".$ids.") and comId=$comId and status=0");
		$fahuo_pici = array();
		...
		foreach ($pdts as $pdt){
			...
			$k_title = '';
			$product_list = array();
			$db->query("update demo_pdt_order set status=3 where id in (".$pdt->orderIds.")");
			$dingdan = $db->get_results("select pdtInfo,num from pdt_order_detail where orderId in (".$pdt->orderIds.")");
