	...
	<div class="jiliangdanwei">
		<div class="jiliangdanwei_up">
			...
			<div class="jiliangdanwei_up_right">
			    <? chekurl($arr,'<a href="javascript:" _href="?m=system&s=product_channel&a=addProductChannel" onclick="edit_channel(0,0,\'\',\'\',0,\'\');">+ 新 增</a>') ?>
			</div>
						...
						<li data-id="<?=$c->id?>" data-pid="0">
							<div class="shangpinguanli_01">
								...
								<div class="shangpinguanli_01_right">
									...
									<? chekurl($arr,'<a href="javascript:" _href="?m=system&s=product_channel&a=addProductChannel" onclick="edit_channel(0,'.$c->id.',\'\',\'\',\'\');"><img src="images/biao_57.png"/> 新增子类</a>') ?>
									<? chekurl($arr,'<a href="javascript:" _href="?m=system&s=product_channel&a=addProductChannel" onclick="edit_channel('.$c->id.',0,\''.$c->title.'\',\''.$c->originalPic.'\',\''.$c->backimg.'\',\''.$c->is_hot.'\',\''.$c->type.'\',\''.$c->remark.'\');"><img src="images/biao_49.png"/> 修改</a>') ?>
									...
									<? chekurl($arr,'<a href="javascript:" _href="?m=system&s=product_channel&a=delChannel" '.($ifdele==1?'onclick="z_confirm(\'确定要删除“'.$c->title.'”分类吗？\',delChannel,'.$c->id.');"':'style="color:#999;opacity:.7"').'><img src="images/biao_48.png"/> 删除</a>') ?>
								</div>
								<div class="clearBoth"></div>
							</div>
							<?
									...
									<ul>
										...
										foreach ($channels1 as $c1) {
											...
											if($cache==1){
												...
												$channels2 = $db->get_results("select * from demo_product_channel where comId=$comId and parentId=".$c1->id." and parentId !=863 order by ordering desc,id asc");
											}
											if(!empty($channels2)){
												$ifdele = 0;
											}else{
											...
											<li data-id="<?=$c1->id?>" data-pid="<?=$c->id?>">
												<div class="shangpinguanli_01">
													<div class="shangpinguanli_01_left" <?if($c1->is_hot) echo 'style="color:red;"'?>>
														<span class="shangpinguanli_01_left_0<? if(empty($channels2) || $c1->title=='仪器耗材'){echo '2';}else{if(in_array($c1->id,$showArry)){echo '3';}else{echo '1';}}?>"></span> 
														<? if($c1->title == '仪器耗材'){ ?>
														   <?=$c1->title?> &nbsp;(如修改，请修改上面的仪器耗材)
														<?}else{?>
														   <?=$c1->title?>
														<? } ?>
													</div>
													<div class="shangpinguanli_01_right">
														...
														<? } ?>
														<a href="?m=system&s=product_channel&a=move&type=1&id=<?=$c1->id?>"><img src="images/biao_33.png"/> 向上</a><a href="?m=system&s=product_channel&a=move&type=0&id=<?=$c1->id?>"><img src="images/biao_34.png"/>向下</a><a href="?m=system&s=product_channel&a=tohot&id=<?=$c->id?>"><a href="javascript:" <? if($ifdele==1){?>onclick="z_confirm('确定要删除“<?=$c1->title?>”分类吗？',delChannel,<?=$c1->id?>);"<? }else{?>style="color:#999;opacity:.7"<? }?>><img src="images/biao_48.png"/> 删除</a>
													</div>
													<div class="clearBoth"></div>
												</div>
												<?
																...
																<li data-id="<?=$c2->id?>" data-pid="<?=$c1->id?>">
																	<div class="shangpinguanli_01">
																		...
																		<div class="shangpinguanli_01_right">
																			...
																			<a href="javascript:" onclick="edit_channel(<?=$c2->id?>,<?=$c1->id?>,'<?=$c2->title?>','<?=$c2->originalPic?>','<?=$c2->backimg?>','<?=$c2->is_hot?>','<?=$c2->type?>','<?=$c2->remark?>');"><img src="images/biao_49.png"/> 修改</a><a href="?m=system&s=product_channel&a=move&type=1&id=<?=$c2->id?>"><img src="images/biao_33.png"/> 向上</a><a href="?m=system&s=product_channel&a=move&type=0&id=<?=$c2->id?>"><img src="images/biao_34.png"/>向下</a><a href="?m=system&s=product_channel&a=tohot&id=<?=$c->id?>"><a href="javascript:" <? if($ifdele==1){?>onclick="z_confirm('确定要删除“<?=$c2->title?>”分类吗？',delChannel,<?=$c2->id?>);"<? }else{?>style="color:#999;opacity:.7"<? }?>><img src="images/biao_48.png"/> 删除</a>
																		</div>
																		<div class="clearBoth"></div>
																	</div>
																	<?
