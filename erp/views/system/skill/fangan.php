...
$allRows = array(
				"title"=>array("title"=>"解决方案名称","rowCode"=>"{field:'title',title:'解决方案名称',width:200}"),
				...
				// "address"=>array("title"=>"招聘地址","rowCode"=>"{field:'address',title:'招聘地址',width:200}"),
...
<body>
	<div class="right_up">
		<img src="images/biao_19.png"/> 服务名称：<?=$skill->title?> - 解决方案列表
	</div>
     ...
     <!--               </div>-->
					<div class="splist_up_01_right">	
						<div class="splist_up_01_right_1">
							<div class="splist_up_01_right_1_left">
								<input type="text" id="keyword" value="<?=$keyword?>" placeholder="请输入解决方案名称/解决方案简介"/>
							</div>
						...
						<div class="splist_up_01_right_3">
						    <? chekurl($arr,'<a href="?&m=system&s=skill&a=addFangan&skillId='.$skill->id.'" class="splist_add">新 增</a>') ?>
						</div>
						<div class="clearBoth"></div>
					</div>
					<div class="clearBoth"></div>
			...
			<div class="splist_down1">
				...
				<div class="yuandian_xx" id="operate_row" data-id="0">
					<ul>
						<? chekurl($arr,'<li><a href="javascript:" _href="?&m=system&s=skill&a=addFangan" onclick="edit_anli()"><img src="images/biao_31.png"> 编辑</a></li>') ?>
			           
			            
						<? chekurl($arr,'<li><a href="javascript:" _href="?&m=system&s=skill&a=delFangan" onclick="z_confirm(\'确定要删除该服务解决方案吗？\',del_anli,\'\');"><img src="images/biao_32.png"> 删除</a></li>') ?>
					</ul>
				</div>
	...
	<script type="text/javascript">
		...
		layui.use(['laydate', 'laypage','table','form'], function(){
		  ...
		  productListTalbe = table.render({
		    elem: '#product_list'
		    ,height: "full-140"
		    ,url: '?m=system&s=skill&a=getFanganList&skillId=<?=$skillId?>'
		    ,page: {curr:<?=$page?>}
		    ,limit:<?=$limit?>
		    ,cols: [[<?=$rowsJS?>]]
		    ,where:{
		...
		function edit_anli(params){
        	...
        	location.href="?m=system&s=skill&a=addFangan&id="+pdtId+"&url="+url;
        }
        ...
        function del_anli(params){
        	...
        	ajaxpost=$.ajax({
        		type: "POST",
        		url: "?&m=system&s=skill&a=delFangan",
        		data: "&ids="+pdtId,
        		dataType:"json",timeout : 8000,
        		success: function(resdata){
        			if(resdata.code==1){
