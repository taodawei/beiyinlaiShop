...
$allRows = array(
				"image"=>array("title"=>"图标","rowCode"=>"{field:'image',title:'图标',width:80}"),
				"title"=>array("title"=>"流程名称","rowCode"=>"{field:'title',title:'流程名称',width:200}"),
                "channelTitle"=>array("title"=>"所属服务","rowCode"=>"{field:'channelTitle',title:'所属服务',width:150}"),
				"jianjie"=>array("title"=>"简介","rowCode"=>"{field:'jianjie',title:'简介',width:120}"),
...
$skill = $db->get_row("select * from demo_skill where id = $skillId");
...
<head>
	...
	<style>
		...
		td[data-field="image"] img{border:#abd3e7 1px solid;height:60px;width:60px;}
...
<body>
	<div class="right_up">
		<img src="images/biao_19.png"/> 服务名称：<?=$skill->title?> - 流程列表
	</div>
     ...
     <!--               </div>-->
					<div class="splist_up_01_right">	
						<div class="splist_up_01_right_1">
							<div class="splist_up_01_right_1_left">
								<input type="text" id="keyword" value="<?=$keyword?>" placeholder="请输入流程名称/流程简介"/>
							</div>
						...
						<div class="splist_up_01_right_3">
						    <? chekurl($arr,'<a href="?&m=system&s=skill&a=addProcess&skillId='.$skill->id.'" class="splist_add">新 增</a>') ?>
						</div>
						<div class="clearBoth"></div>
					</div>
					<div class="clearBoth"></div>
			...
			<div class="splist_down1">
				...
				<div class="yuandian_xx" id="operate_row" data-id="0">
					<ul>
						<? chekurl($arr,'<li><a href="javascript:" _href="?&m=system&s=skill&a=addProcess" onclick="edit_anli()"><img src="images/biao_31.png"> 编辑</a></li>') ?>
			           
			            
						<? chekurl($arr,'<li><a href="javascript:" _href="?&m=system&s=skill&a=delProcess" onclick="z_confirm(\'确定要删除该服务流程吗？\',del_anli,\'\');"><img src="images/biao_32.png"> 删除</a></li>') ?>
					</ul>
				</div>
	...
	<script type="text/javascript">
		...
		layui.use(['laydate', 'laypage','table','form'], function(){
		  ...
		  productListTalbe = table.render({
		    elem: '#product_list'
		    ,height: "full-140"
		    ,url: '?m=system&s=skill&a=getProcessList&skillId=<?=$skillId?>'
		    ,page: {curr:<?=$page?>}
		    ,limit:<?=$limit?>
		    ,cols: [[<?=$rowsJS?>]]
		    ,where:{
		...
		function edit_anli(params){
        	...
        	location.href="?m=system&s=skill&a=addProcess&id="+pdtId+"&url="+url;
        }
        ...
        function showCard(params)
        {
        	...
        	location.href="?m=system&s=skill&a=fangan&skill="+pdtId+"&url="+url;
        }
        ...
        function del_anli(params){
        	...
        	ajaxpost=$.ajax({
        		type: "POST",
        		url: "?&m=system&s=skill&a=delProcess",
        		data: "&ids="+pdtId,
        		dataType:"json",timeout : 8000,
        		success: function(resdata){
        			if(resdata.code==1){
