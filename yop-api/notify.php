...
if($result['status']=='SUCCESS' && $result['payAmount']>0){
	...
	$order = $db->get_row("select * from order$fenbiao where orderId='".str_replace('__','_',$result['orderId'])."' limit 1");
	if($order->status==-5||$order->status==-1){
		...
		if($order->price_dingjin==0){
			...
			$pay_json['dingjin']['paytype'] = '易宝，订单号：'.$result['uniqueOrderNo'];
			$pay_json['dingjin']['orderId'] = $result['orderId'];
			$pay_json['dingjin']['pay_way'] = $result['paymentProduct'];
		}
...
function diancan_pay_done($orderId,$userId,$comId){
	...
	if(!empty($pay_json['yibao']['price'])){
		...
		$db->query("update demo_yibao_fenzhang set status=2 where comId=$order->comId and orderId=$order->id and comId=$order->comId and status=1 limit 1");
	}
	return true;
}
//订单付完全款执行的方法
	...
	if($order->peisong_type==4){
		...
		if($order->tuan_id>0){
					...
					if($tuan->type==2){
						...
						foreach ($oids as $oid){
							...
							if($if_tongbu==1){
								...
								$db->query("update demo_yibao_fenzhang set status=1 where comId=$order_comId and orderId=$oid and payId=0 limit 1");
							}
			...
			if($if_tongbu==1){
				...
				$db->query("update demo_yibao_fenzhang set status=1 where comId=$order_comId and orderId=$orderId and payId=0 limit 1");
			}
