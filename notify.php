...
if(!empty($fhData->out_trade_no)){
	...
	$order = $db->get_row("select * from order$fenbiao where id=$payLog->typeId limit 1");
	if(($order->status==-5||$order->status==-1) && empty($order->qx_time)){
		$orderId = $order->id;
		$o = array();
		$o['id'] = $orderId;
		$o['pay_types'] = $order->pay_types.",3";
		if($order->price_dingjin==0){
			$o['status'] = empty($order->tuan_id)?2:0;//普通订单要设置为待发货状态，并且添加发货单
			$o['ispay'] = 1;
			$o['pay_type'] = 2;
		}
		$o['price_payed'] = bcadd($total_fee, $order->price_payed,2);
		...
		if($order->price_dingjin==0){
			$pay_json['applet']['price'] = $total_fee;
			$pay_json['applet']['desc'] = $transaction_id;
		}else{
