...
class Connection
{
    ...
    */
    ...
    protected function execute($query, $parameters = "")
    {
        try {
            ...
            if ($e->errorInfo[1] == 2006 || $e->errorInfo[1] == 2013) {
                ...
                file_put_contents('mysql_log/'.date("y-m-d").'.logs', $err_msg.PHP_EOL, FILE_APPEND);
     ...
     * @return array
     */
    public function get_row($query = '', $params = null, $fetchmode = PDO::FETCH_OBJ)
    {
        $query = trim($query);
        if (empty($query)) {
            $query = $this->build();
