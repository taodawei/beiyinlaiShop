...
class Product{
	...
	public function searchPlist()
	{
		...
		//先把只有product
		$products = $db->get_var("select group_concat(p.id) from demo_product p left join demo_product_inventory i on  i.productId = p.id where i.productId is null ");
		if($products){
		    $db->query("delete from demo_product where id in ($products) ");
		}
...
// 		echo str_replace('i.productId,i.id as inventoryId,i.price_sale as price_sale,i.fanli_shequ as fanli_shequ,i.orders as orders,i.views as views,i.title,i.dtTime,i.image,i.price_market,i.comId,i.ordering','count(distinct(i.id))',$sql);die;
		...
		if(!empty($pdts)){
			foreach ($pdts as $i=>$pdt) {
				...
				$pro = $db->get_row("select brandId,price_name,originalPic,untis,remark,subtitle,yunfei_moban_ding,skuId,skuDay,channelId,book_url,originalPic,skuId,tags from demo_product where id=$pdt->productId");
				...
				$data['paper_num'] = 0;
				$data['tags'] = $pro->tags ? explode(',', $pro->tags) : [];
				if($pro->skuId){
				...
				//david 25-10-24 增加字段 inventorys[]
                $inventorys = array();
                $keys = $db->get_results("select * from demo_product_inventory where productId=$pdt->productId order by id asc");
                if(count($keys)>1){
                    foreach ($keys as $key){
                        $k = array();
...
// 		}
    ...
    function detail(){
		...
		if(isset($request['skuId'])){
		    $skuId = $request['skuId'];
		    $product = $db->get_row("select * from demo_product where skuId = '$skuId' ");
		    
		    if(empty($product)){
    			return '{"code":0,"message":"商品不存在或已下架"}';
    		}
    		
    		$id = $inventoryId = (int)$db->get_var("select id from demo_product_inventory where productId = $product->id and status = 1 order by price_sale asc, id asc ");
