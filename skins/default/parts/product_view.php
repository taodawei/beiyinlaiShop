...
$product_inventory = $db->get_row("select * from demo_product_inventory where id=$id and comId=$comId and status=1");
...
if($xinren==1){
    $if_order = $db->get_var("select id from order$fenbiao where comId=$comId and userId=$userId and status!=-1 limit 1");
    if($if_order>0){
        echo '<script>alert("只有没有下过单的会员可以享受新人专享优惠！您不符合条件。");go_prev_page();</script>';
        exit;
    }
    $price_sale = $db->get_var("select money from demo_xinren_discount where inventoryId=$id and comId=$comId limit 1");
    if(empty($price_sale)){
...
if($comId==1142&&$_SESSION[TB_PREFIX.'user_level']!=118){
    $product_inventory->title = $product->subtitle;
}
?>
...
<div id="chanpin">
    <div class="chanpin_1">
        <ul id="pdtMenu">
            <li>
                <a href="javascript:" onclick="qiehuan('pdt',1,'chanpin_1_on');$('#pdtCont2').show();$('.chanpin_7').show();" id="pdtMenu1" class="chanpin_1_on">商品</a>
            </li>
            <li>
                <a href="javascript:" onclick="qiehuan('pdt',2,'chanpin_1_on');show_imgs();$('.chanpin_7').show();" id="pdtMenu2">详情</a>
            </li>
            <li>
                <a href="javascript:" onclick="qiehuan('pdt',3,'chanpin_1_on');init_pingjia();$('.chanpin_7').hide();" id="pdtMenu3">评价</a>
            </li>
    ...
    <div class="pdtCont" id="pdtCont1">
        <div class="chanpin_2">
            ...
            <? if(empty($yushou)){?>
                <div class="chanpin_2_04" <? if($comId==1142&&$_SESSION[TB_PREFIX.'user_level']!=118){?>style="display:none"<? }?>>  
                ...
                <? if(!empty($product_inventory->price_card)){
                    if($comId==1113){?>
                        <div class="chanpin_2_05"><span>零售价</span> ¥<font id="price_user"><?=getXiaoshu($product_inventory->price_sale,2)?></font></div>
                    <? }else{
                    ?>
                    <div class="chanpin_2_05" <? if($comId==1142&&$_SESSION[TB_PREFIX.'user_level']!=118){?>style="display:none"<? }?>><span>会员价</span> ¥<font id="price_user"><?=getXiaoshu($price_sale-$product_inventory->price_card,2)?></font></div>
                <? }
                }
                ?>
            <? }else{
        ...
        if($product_inventory->sale_tuan==1 && $product_inventory->tuan_num){
            ...
            if(!empty($tuangous)){
            ?>
            <div class="pintuanxx_3">
                <div class="pintuanxx_3_up">
                    <div class="pintuanxx_3_up_left">
                        <?=count($tuangous)?>人在拼单，可直接参与
                    </div>
                    <div class="pintuanxx_3_up_right">
                         <a href="javascript:" onclick="$('#pintuanxx_more_tc').show();">查看更多 ></a>
                    </div>
                    <div class="clearBoth"></div>
                </div>
                <div class="pintuanxx_3_down">
                    <ul>
                        <?
                        $tuangou = $tuangous[0];
                        if(!empty($tuangou)){
                            $u = $db->get_row("select nickname,image from users where id=$tuangou->tuanzhang");
                            ?>
                            <li>
                                <div class="pintuanxx_3_down_1">
                                    <img src="<?=ispic($u->image)?>"/> <?=sys_substr($u->nickname,4,true)?>
                                </div>
                                <div class="pintuanxx_3_down_2">
                                    <h2>还差<?=$tuangou->user_num-$tuangou->nums?>人拼成</h2>
                                    剩余<span id="jishiqi11">00:00:00</span>
                                </div>
                                <div class="pintuanxx_3_down_3">
                                    <a href="/index.php?p=4&a=cantuan&id=<?=$id?>&tuanId=<?=$tuangou->id?>">去参团</a>
                                </div>
                                <div class="clearBoth"></div>
                            </li>
                            <script type="text/javascript">
                                $(function(){
                                    countDown('<?=strtotime($tuangou->endTime)?>',11);
                                });
                            </script>
                            <?
                        }
                        ...
                        if(!empty($tuangou)){
                            ...
                            <li>
                                ...
                                <div class="pintuanxx_3_down_2">
                                    <h2>还差<?=$tuangou->user_num-$tuangou->nums?>人拼成</h2>
                                    剩余<span id="jishiqi12">00:00:00</span>
                                </div>
                                <div class="pintuanxx_3_down_3">
                                    <a href="/index.php?p=4&a=cantuan&id=<?=$id?>&tuanId=<?=$tuangou->id?>">去参团</a>
                                </div>
                            ...
                            <script type="text/javascript">
                                $(function(){
                                    countDown('<?=strtotime($tuangou->endTime)?>',12);
                                });
                            </script>
                            <?
                        }
    ...
    <div class="shangpinjieshao pdtCont" id="pdtCont2" >
        ...
        <div class="shangpinjieshao_down pdtContCont" id="pdtContCont1">
            ...
            $cont1 = str_replace('<img src="', '<img class="lazy" data-original="', $cont1);
            echo $cont1;
            ?>
        </div>
        <div class="shangpinjieshao_down pdtContCont" id="pdtContCont2" style="display:none;">
            ...
            $cont2 = str_replace('<img src="', '<img class="lazy" data-original="', $cont2);
            echo $cont2;
            ?>
        </div>
        <div class="shangpinjieshao_down pdtContCont" id="pdtContCont3" style="display:none;">
            ...
            $cont3 = str_replace('<img src="', '<img class="lazy" data-original="', $cont3);
            echo $cont3;
            ?>
        </div>
    </div>
    ...
    <div class="chanpin_9">
        <div class="chanpin_9_left">
            <ul>
                <li>
                    <a <? if(!empty($shezhi->com_kefu)){?>href="<?=$shezhi->com_kefu?>"<? }else{?>href="javascript:" onclick="$('#cp_kefu_tc').show();"<? }?>><img src="/skins/default/images/shangpinxx_22.png"/><br>联系客服</a>
                </li>
                <li>
        ...
        <div class="chanpin_9_right">
            <ul>
                <? if(empty($yushou) && empty($xinren) && empty($lingyuangou)){
                    //是否拼团
                    if($product_inventory->sale_tuan==1 && $product_inventory->tuan_num){?>
                        <li>
                        ...
                        <li>
                            <a href="javascript:" onclick="show_buy_div(2,1);" class="chanpin_9_right_02" style="line-height:.875rem;padding-top:.3rem;">
                                ￥<?=$product_inventory->price_tuan?><br>我要开团</a>
                        </li>
                    <? }else{?>
                        <li>
...
<!--更多拼团-->
<div class="pintuanxx_more_tc" id="pintuanxx_more_tc" style="display:none;">
    <div class="bj" onclick="$('#pintuanxx_more_tc').hide();">
    </div>
    <div class="pintuanxx_more">
        <div class="pintuanxx_more_1">
            正在拼单
            <div class="pintuanxx_more_1_right" onclick="$('#pintuanxx_more_tc').hide();">
                <img src="/skins/erp_zong/images/pintuan_26.png"/>
            </div>
        </div>
        <div class="pintuanxx_more_2">  
            <ul>
                <?
                if(!empty($tuangous)){
                    foreach ($tuangous as $tuangou) {
                        $u = $db->get_row("select nickname,image from users where id=$tuangou->tuanzhang");
                        ?>
                            <li>
                                <div class="pintuanxx_3_down_1">
                                ...
                                <div class="pintuanxx_3_down_2">
                                    <h2>还差<?=$tuangou->user_num-$tuangou->nums?>人拼成</h2>
                                    剩余<span id="jishiqi12<?=$tuangou->id?>">00:00:00</span>
                                </div>
                                <div class="pintuanxx_3_down_3">
                                    <a href="/index.php?p=4&a=cantuan&id=<?=$id?>&tuanId=<?=$tuangou->id?>">去参团</a>
                                </div>
                            ...
                            <script type="text/javascript">
                                $(function(){
                                    countDown('<?=strtotime($tuangou->endTime)?>',12<?=$tuangou->id?>);
                                });
                            </script>
                        <?
                    }
...
<div class="cp_lijigoumai_tc" id="cp_lijigoumai_tc" style="display:none;">
    ...
    <div class="cp_lijigoumai">
        ...
        <?
        if($_SESSION['if_shequ_tuan']==1){
        ?>
        <div class="pintuanxx_kaituan_2" id="tuan_type_div" style="display:none;">
            <div class="pintuanxx_kaituan_2_up">
                请选择拼团形式
            </div>
            <div class="pintuanxx_kaituan_2_down">
                <ul>
                    <li>
                        <a href="javascript:" onclick="huan_tuan_type(1);" class="pintuanxx_kaituan_2_down_on"> <img src="/skins/erp_zong/images/pintuan_29.png"> 普通团</a>
                    </li>
                    <li>
                        <a href="javascript:" onclick="huan_tuan_type(2);"> <img src="/skins/erp_zong/images/pintuan_29.png"> 社区团</a>
                    </li>
                    <div class="clearBoth"></div>
                </ul>
            </div>
...
<script type="text/javascript">
    ...
    var share_url = 'http://<?=$_SERVER['HTTP_HOST']?>/index.php?<?=$_SERVER["QUERY_STRING"]?>&tuijianren=<?=$userId?>';
...
<script src="/skins/default/scripts/product/product_view.js?v=1.4"></script>