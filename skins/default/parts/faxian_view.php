...
global $db,$request;
$id = (int)$request['id'];
$faxian = $db->get_row("select * from demo_faxian where id=$id");
if(empty($faxian)){
    die('文章不存在');
}
$db->query("update demo_faxian set views=views+1 where id=$faxian->id");
...
<?
require(ABSPATH.'/skins/erp_zong/bottom.php');
?>
<script type="text/javascript">
    var share_url = 'http://<?=$_SERVER['HTTP_HOST']?>/index.php?p=9&a=view&id=<?=$id?>&tuijianren=<?=(int)$_SESSION[TB_PREFIX.'user_ID']?>';
    var share_title = '<?=$faxian->title?>';
    var share_img = '<?=$imgs[0]?>';
    var share_desc = '<?=$faxian->remark?>';
    $(function(){
