                  ...
                  '<a onclick="setInitScroll();" href="/index.php?p=4&a=view&id='+item.inventoryId+'">'+
                      ...
                      '<div class="shangpinlist_tt">'+
                        '<div class="shangpinlist_tt_01">  '+
                            item.title+
                        '</div>'+
                        '<div class="shangpinlist_tt_02 '+(item.hidden==1?'hidden':'')+'">  '+
	...
	lay_flow.load({
        ...
        ,done: function(page, next){
        	...
        	$.ajax({
        		...
        		data: "channelId="+channelId+"&yhq_id="+yhq_id+"&cuxiao_id="+cuxiao_id+"&keyword="+keyword+"&tags="+tags+"&order1="+order1+"&order2="+order2,
        		...
        		success: function(res){
                    ...
                    '<a onclick="setInitScroll();" href="/index.php?p=4&a=view&id='+item.inventoryId+'">'+
                        ...
                        '<div class="shangpinlist_tt">'+
                          ...
                          '</div>'+
                          '<div class="shangpinlist_tt_02 '+(item.hidden==1?'hidden':'')+'">  '+
                            (item.price_name1==''?'':item.price_name1+'：')+'￥'+item.price_sale+' <span>'+item.price_name+'：￥'+item.price_market+'</span>'+
                          '</div>'+
                        '</div>'+
                        '<div class="clearBoth"></div>'+
                      '</a>'+
