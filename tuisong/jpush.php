...
class jpush {
	...
	function send($sendno = 0,$receiver_type = 1, $receiver_value = '', $msg_type = 1, $msg_content = '', $platform = 'android') {
		...
		$sql = "INSERT INTO  ".DB_NAME.".`".DB_TAB."` (`id` ,`sendno` ,`n_title` ,`n_content` ,`errcode` ,`errmsg` ,`total_user` ,`send_cnt` ,`created`)VALUES ( NULL ,'".$sendno."','".$msg_content['n_title']."','".$msg_content['n_content']."', '".$res_arr['errcode']."', '".$res_arr['errmsg']."', '0','0','".$created."')";
		global $db;
		$query = $db->query($sql);		
				
		if (intval($res_arr['errcode'])==0){	
				$str= "第".$sendno."条发送".$res_arr['errmsg']."！";
			}else{						
				$str= "第".$sendno."条发送失败：".$res_arr['errmsg'];
			}
	   return $str;
	}
